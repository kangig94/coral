...
**요구사항:** Linux 또는 macOS, 그리고 Node.js 26. Node는 24 이상이면 동작할 가능성이 높지만 검증하지 않습니다. Windows는 지원하지 않습니다.
...
| `/coral:loop-review`   | tier별로 BLOCKING이 없어질 때까지 리뷰와 수정을 반복한 뒤, 마지막에 tier-review를 한 번 실행 |      ✓       |
...
| `CORAL_CODEX_MODEL`           | `gpt-6-sol`          | Codex CLI 기본 모델. 크기별 baseline(GPT-5.6 계열 또는 `gpt-6-astra`/`sol`/`luna`)에서는 abstract tier를 `fable`→`astra`, `opus`→`sol`, `sonnet`→`terra`, `haiku`→`luna`로 매핑하고, `astra`/`sol`/`terra`/`luna` 짧은 이름도 받음; 그 외(예: `gpt-5.5`)는 size 구분 없이 해당 baseline 하나로 통일 |
| `CORAL_CODEX_EFFORT`          | `high`               | Codex 추론 노력도 (`low`…`ultra`). 상한: Astra/Sol/Terra `ultra`, Luna `max`, gpt-5.5 `xhigh`; Terra/Luna 하한 `xhigh`                                                                                                                                                                              |
...
| `CORAL_CLAUDE_MODEL_CAP`      | `fable`              | Claude 최대 모델 티어 (`fable` > `opus` > `sonnet` > `haiku`)                                                                                                                                                                                                                                       |
...
| `CORAL_SYSTEM_PROVIDER_SCOPE` | _(없음)_             | HTTP와 데몬 내부 provider 작업에 쓰는 이름 있는 strict system profile scope. 설정하지 않으면 해당 실행 경로가 꺼지고, 바꾸면 데몬을 재시작해야 합니다 ([상세](docs/configuration.md#multi-account-provider-routing))                                                                                |
