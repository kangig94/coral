...
| `CORAL_CODEX_MODEL`           | `gpt-5.6-sol`        | Codex CLI 기본 모델. 크기별 baseline(GPT-5.6 계열 또는 `gpt-6-astra`)에서는 abstract tier를 `fable`→`astra`, `opus`→`sol`, `sonnet`→`terra`, `haiku`→`luna`로 매핑하고, `astra`/`sol`/`terra`/`luna` 짧은 이름도 받음; 그 외(예: `gpt-5.5`)는 size 구분 없이 해당 baseline 하나로 통일 |
