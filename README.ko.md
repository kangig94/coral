...
- **시맨틱 검색**: `/coral:equip kb`로 벡터 검색 활성화 — BM25 + 임베딩 하이브리드 검색 (Gemini, OpenAI, 로컬 ONNX 모델)
...
| `CORAL_CODEX_FAST` | _(없음)_ | Codex 서비스 티어 (`fast` / `flex`, `1/true/0/false` 허용). 미설정 시 `~/.codex/config.toml` 최상위 `service_tier`로 폴백 |
