...
Coral이 프로젝트를 읽고 구조화된 분석 결과를 반환합니다: 아키텍처, 모듈, 진입점,
의존성 그래프. 파일 생성 없음, 설정 불필요.
...
컨벤션, 에이전트, 아키텍처 문서를 프로젝트에 맞게 생성합니다. Claude가 일반적인
기본값이 아닌 당신의 규칙을 따르게 됩니다.
...
어디서 본 구조 아닌가요? 이 저장소의 `.claude/` 폴더가 바로 그 결과물입니다.
...
다양한 AI 페르소나가 각자의 관점에서 논쟁합니다. 할 말이 있는 쪽이 먼저 발언하는
입찰 시스템. 상호 검증을 거치며 입장이 정제됩니다. 최종 종합으로 마무리.
트랜스크립트는 `.claude/coral/discuss/`에 저장.
...
현상학자, 계산 신경과학자, AI 안전 연구자, 로봇공학 엔지니어, 동양철학 학자가
LLM이 AGI인지 토론합니다. 5명의 에이전트, 15개의 발언, 3개의 수렴점.
...
패널의 수렴점: LLM은 AGI가 아니지만 **전혀 새로운 유형의 시간적 존재** — 자신의
시간 척도 안에서는 놀라운 역량을 보이지만, 구조적 경계에서의 행동은 미지수.
...
아키텍트와 크리틱 리뷰를 포함한 구조화된 계획. 체계적 버그 진단 — 근본 원인, 계획,
수정. 완료 전까지 반복 검증하며 실행.
...
Coral은 프로젝트 로컬 지식 베이스를 `.claude/coral/kb/`에 유지합니다 — git으로
추적되어 기기 간 동기화됩니다.

Claude가 놓치기 쉬운 것을 발견하면 (근본 원인, 주의사항, 기억할 패턴) 즉시 메모를
작성합니다. 에러 발생 시 처음부터 디버깅하기 전에 KB를 확인합니다. 작업 완료 시
메모를 검토하고 영구 항목으로 승격합니다. 같은 실수를 세션마다 반복하지 않습니다.
