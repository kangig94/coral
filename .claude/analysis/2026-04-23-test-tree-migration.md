...
Historical note: this document records the earlier in-`src` migration phases and therefore preserves original `src/**/__tests__` paths in the body. The final rewrite layout keeps executable test code under `tests/**` and reusable test support under `tests/helpers/**`.
