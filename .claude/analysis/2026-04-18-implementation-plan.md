...
- **A9 — Provider plugin architecture supersedes A1–A8 slot model** (Phase 7 redesign, 2026-04-27). The initial §2.8a draft (slot + provider + expansion-contract with priority and explicit `isReady`/`waitForReady`) was authored 2026-04-27 morning and superseded the same day by the meta-pioneer synthesis. The unified design uses three primitives: (1) `RuntimeBinding<T>` — a domain-owned mutable cell with structural single-occupancy and an optional default value (replaces `Slot`/`SlotProvider`/`SlotRegistry`); (2) `Plugin = (host: PluginHost) => void | Promise<void>` — a single-function contract for everything that participates (replaces `ExpansionContract`'s four-method interface); (3) `Provider<T> = { read(): T, consumer: Consumer }` — capability + freshness, where the cell holds `Provider<T>` (not raw `T`) so routing reads `binding.read().read(...)` while `waitFreshUntil` reads `binding.read().consumer.id` from the same cell. **Rationale**: Coral has exactly one decision per binding (which implementation runs now), not a multiplexer over competing implementations. Priority, `isReady()`, `waitForReady()`, slot lifecycle methods, default-vs-override asymmetry all collapse out. Cross-references: architecture §2.8a (full contract), §2.8 (prose updated 2026-04-27 for plugin-registry vocabulary), §16 invariants #43 (rewritten — structural single-occupancy), #43a (Orama is default value, not a plugin), #43b (Plugin function shape), #43c (readiness is comparison). Migration order is 7 commits, each independently green; commit 1 is `RuntimeBinding<T>` primitive in `runtime/binding.ts`. This amendment renders A1's "/equip uninstall" slash-command form unaffected, A2's needle identity unification unaffected, A3's `equipment_state` table re-keyed by plugin id (not slot id), A4's `ConsumerHandle` shape now exposed via `Provider<T>.consumer`, A5/A6 unaffected, A7's `waitFreshUntil` now the *only* readiness primitive (formerly one of two), and A8's source-import readiness routed through `runtime.authorities.corpus.waitFreshUntil(V, kbRuntime.vector.read().consumer.id)`.
