...
- **[SUPERSEDED BY A9] A1 — `/equip uninstall` replaces `/unequip` slash command** (Phase 7, tag `phase-7`). Rationale: `/equip` is already the catalog + lifecycle entry; a separate slash command duplicates surface area for no UX gain. Coordinator RPC name `unregisterEquipment` is preserved; only the CLI surface collapses.
...
- **[SUPERSEDED BY A9] A7 — Corpus freshness uses the same waiter primitive as Journal freshness** (Phase 7 amendment). Invariant 41 now requires `waitFreshUntil(authority, version, consumerId)` for both Journal and Corpus consumers. `listEquipment` remains a status/read-model surface, not the mechanism for source-import readiness.
...
- **A9 — Expansion architecture supersedes A1–A8 slot model** (Phase 7 redesign, 2026-04-27). The initial §2.8a draft (slot + provider + expansion-contract with priority and explicit `isReady`/`waitForReady`) was authored 2026-04-27 morning and superseded the same day by the meta-pioneer synthesis. The unified design uses three primitives: (1) `RuntimeBinding<T>` — a domain-owned mutable cell with structural single-occupancy and an optional default value (replaces `Slot`/`SlotProvider`/`SlotRegistry`); (2) `Expansion = (host: ExpansionHost) => void | Promise<void>` — a single-function contract for everything that participates (replaces `ExpansionContract`'s four-method interface); (3) `Backed<T> = { read(): T, consumer: Consumer }` — capability + freshness, where the cell holds `Backed<T>` (not raw `T`) so routing reads `binding.read().read(...)` while `waitFreshUntil` reads `binding.read().consumer.id` from the same cell. **Rationale**: Coral has exactly one decision per binding (which implementation runs now), not a multiplexer over competing implementations. Priority, `isReady()`, `waitForReady()`, slot lifecycle methods, default-vs-override asymmetry all collapse out. `kb.embedding` has no constructor-time default; embedders are peer Expansions discriminated by `BundledExpansion.metadata.slot === 'kb.embedding'`, and `BUNDLED_EXPANSIONS` carries ≥1 embedder entry so the binding is fillable. Cross-references: architecture §2.8a (full contract), §2.8 (Skill / Expansion taxonomy), §16 invariants #43 (structural single-occupancy), #43a (Orama is default value, not an Expansion), #43b (Expansion function shape), #43c (`Backed<T>` readiness is comparison), #43d (peer-category `kb.embedding`). Migration order is 8 commits, each independently green; commit 0 is the doc vocabulary correction, commit 1 is `RuntimeBinding<T>` primitive in `runtime/binding.ts`. The Round-3 cascade-break Adapt remains part of the plan: legacy `src/expansion/{contracts, equipment-contract, catalog, errors, activate, workflow, install, strategies}.ts` files are intentionally retained through commits 2-5 and batch-deleted in commit 6 so importer flips stay per-commit green. This amendment renders A1's "/equip uninstall" slash-command form historical only, A2's needle identity unification unaffected, A3's durable state rewritten as `expansion_state` keyed by expansion id (not slot id) plus `consumer_cursors`, A4's `ConsumerHandle` shape surfaced through `Backed<T>.consumer`, A5/A6 unaffected, A7's `waitFreshUntil` now the *only* readiness primitive (formerly one of two), and A8's source-import readiness routed through `runtime.authorities.corpus.waitFreshUntil(V, kbRuntime.vector.read().consumer.id)`.

Historical note: A1–A8 remain in this file as the pre-A9 record. Headers prefixed `[SUPERSEDED BY A9]` are retained for traceability only.
