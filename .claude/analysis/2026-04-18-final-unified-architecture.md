...
Direct KB reads are structurally read-only. The query edge opens a `ReadonlyDatabase` through `openReadOnlyStoreDatabase(... readonly: true)` ([`src/store/read-port.ts:29`](../../src/store/read-port.ts#L29), [`src/store/read-port.ts:54`](../../src/store/read-port.ts#L54)-[`src/store/read-port.ts:69`](../../src/store/read-port.ts#L69)). The KB-domain wrapper `KbReadPort = { db: ReadonlyDatabase }` lives at [`src/kb/read-port.ts:11`](../../src/kb/read-port.ts#L11); `searchKnowledgeBase` builds its read runtime from that port and loads bundled search capabilities through the read-side expansion host ([`src/kb/queries.ts:31`](../../src/kb/queries.ts#L31)-[`src/kb/queries.ts:38`](../../src/kb/queries.ts#L38), [`src/kb/query-runtime.ts:117`](../../src/kb/query-runtime.ts#L117)-[`src/kb/query-runtime.ts:130`](../../src/kb/query-runtime.ts#L130), [`src/kb/query-runtime.ts:139`](../../src/kb/query-runtime.ts#L139)-[`src/kb/query-runtime.ts:208`](../../src/kb/query-runtime.ts#L208)). A missing Orama snapshot on the read path calls `loadReadOnly()`, reports `fts_index_uninitialized`, and installs only an in-memory fallback; the durable `persist(...)` path is reached from `CorpusConsumer.apply`, not from search ([`src/engines/orama/backend.ts:126`](../../src/engines/orama/backend.ts#L126)-[`src/engines/orama/backend.ts:150`](../../src/engines/orama/backend.ts#L150), [`src/engines/orama/backend.ts:248`](../../src/engines/orama/backend.ts#L248)-[`src/engines/orama/backend.ts:250`](../../src/engines/orama/backend.ts#L250), [`src/engines/orama/snapshot.ts:82`](../../src/engines/orama/snapshot.ts#L82)-[`src/engines/orama/snapshot.ts:97`](../../src/engines/orama/snapshot.ts#L97), [`src/engines/orama/snapshot.ts:99`](../../src/engines/orama/snapshot.ts#L99)-[`src/engines/orama/snapshot.ts:108`](../../src/engines/orama/snapshot.ts#L108)).
...
**`correlationId` + `causationSeq`**: the causality graph. `correlationId` groups related events (same user command). `causationSeq` is reserved as the **general-purpose backward pointer** for arbitrary cross-event causation (request/response correlation, multi-step domain flows). Current implementation uses it sparingly: fault propagation goes through the typed `causeRef` channel on `TerminalOutcome.failed` (which carries `{ stream, seq }` — strictly more structured than the bare `causationSeq` int), and no current code path sets `causationSeq` in production. The field stays in the envelope shape because *any* future flow that wants a typed backward pointer without inventing a new envelope field will use it; introducing it after the fact would cost a `bodyVersion` bump on every existing event type, while leaving it in costs one optional `INTEGER` column.
...
- Stream prefix references (`refs.workflowSlotId`) are rejected at append if the `slotId` is not present in the plan. Child launches currently carry `refs.workflowSlotId` from job launch storage into job projection rows ([`src/jobs/store.ts:420`](../../src/jobs/store.ts#L420), [`src/jobs/projections.ts:171`](../../src/jobs/projections.ts#L171)); Phase 2 adds the append validator coverage in `tests/unit/workflow/plan-validity.test.ts`.
...
37. An Expansion loads via dynamic import (`await import(entry.specifier)` from [`src/expansion/bundled.ts:43`](../../src/expansion/bundled.ts#L43)); its **runtime engine code and heavy dependencies** (native addons, large libraries like `@orama/orama`, vector model runtimes) enter the process only after `/equip` completes. Lightweight install bookkeeping that the manifest references synchronously (e.g., the `installer.inspect(runtime, name)` filesystem-state probe) may load eagerly with the manifest because it is not a heavy dependency. The boundary is whether the module pulls runtime work into the process — `installer` modules only do filesystem checks; engine `expansion.ts` modules are the dynamic-import boundary.
