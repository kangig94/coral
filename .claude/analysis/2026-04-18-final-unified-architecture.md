...
`contentHash` policy:
- `contentHash` is REQUIRED whenever a Journal fact captures point-in-time semantic context: cause-ref payloads whose describer may later render KB evidence through the causal chain ([`src/causality/render.ts:150`](../../src/causality/render.ts#L150)), terminal records or terminal-causing domain progress that preserve why a job ended ([`src/store/append.ts:162`](../../src/store/append.ts#L162), [`src/causality/render.ts:106`](../../src/causality/render.ts#L106)), and cited evidence stored as KB entry references rather than ephemeral search output. Phase 1 is docs-only; the future invariant test `tests/invariants/kb-ref-policy.test.ts` enumerates `KbRef` constructors and asserts capture-time vs. read-time policy alignment.
- `contentHash` is OPTIONAL for transient query results and list views. Search/list responses are current read-model materializations, not durable point-in-time references: `KbResult` carries rendered fields without `contentHash` ([`src/kb/entry-types.ts:52`](../../src/kb/entry-types.ts#L52)), search responses map current hits into result rows ([`src/kb/search/responses.ts:187`](../../src/kb/search/responses.ts#L187)), and list views read the current index directly ([`src/kb/ops/principles-list.ts:62`](../../src/kb/ops/principles-list.ts#L62)).
- Missing-entry render rule: if a `KbRef` points at a deleted Corpus entry, the describer surfaces structured `kb_ref_missing { entryId, capturedAt? }` diagnostic and never renders a silent empty. Future enforcement: `tests/invariants/kb-ref-policy.test.ts` plus the Phase 2 read-port describer tests.
- Hash-mismatch render rule: if the current Corpus entry exists but its hash differs from the captured hash, the describer surfaces `kb_ref_drifted { entryId, capturedHash, currentHash }` and falls back to current content with the diagnostic attached. Future enforcement: `tests/invariants/kb-ref-policy.test.ts` plus the Phase 2 read-port describer tests.
...
22. Cross-authority references use `KbRef = { entryId, contentHash? }` ([`src/store/envelope.ts:20`](../../src/store/envelope.ts#L20)). `contentHash` is required for point-in-time semantic context (cause-ref payloads, terminal records, cited evidence) and optional for transient query results/list views; the future invariant test `tests/invariants/kb-ref-policy.test.ts` enumerates constructors and asserts capture-time vs. read-time policy alignment. Missing entries render `kb_ref_missing { entryId, capturedAt? }`, never silent empty; hash drift renders `kb_ref_drifted { entryId, capturedHash, currentHash }` and falls back to current content with the diagnostic attached. Hash format is SHA-256 of canonical content surface bytes, matching `computeContentSurfaceHash` ([`src/kb/corpus/snapshot.ts:83`](../../src/kb/corpus/snapshot.ts#L83)).
