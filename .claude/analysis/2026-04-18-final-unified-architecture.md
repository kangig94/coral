...
`contentHash` policy:
- `contentHash` is REQUIRED whenever a Journal fact captures point-in-time semantic context: cause-ref payloads whose describer may later render KB evidence through the causal chain ([`src/causality/render.ts:149`](../../src/causality/render.ts#L149)), terminal records or terminal-causing domain progress that preserve why a job ended ([`src/store/append.ts:163`](../../src/store/append.ts#L163), [`src/causality/render.ts:106`](../../src/causality/render.ts#L106)), and cited evidence stored as KB entry references rather than ephemeral search output. Phase 1 is docs-only; the future invariant test `tests/invariants/kb-ref-policy.test.ts` enumerates `KbRef` constructors and asserts capture-time vs. read-time policy alignment.
