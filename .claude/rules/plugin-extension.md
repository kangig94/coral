...
- **Agents**: spawned via `Agent({ subagent_type: "coral:<name>" })` (Claude-native; SubagentStart inject) or `coral-cli codex|claude <name> -i -` (provider job; prompt on stdin; applyInjectBundle). The framework resolves agent files — do not read agent files directly from skills.
