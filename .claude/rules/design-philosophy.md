# Design Philosophy

## Core Principles

1. **MCP Protocol Correctness**: Every tool response follows the MCP SDK contract. Tool handlers return `{ content: [{ type: "text", text }], isError }`. Schema validation via Zod runs before any execution logic.

2. **Stdio Safety**: The MCP server communicates over stdio. Never use `console.log` in server code — it corrupts the MCP transport. Use `process.stderr.write` for diagnostics.

3. **Graceful Degradation**: If Codex CLI is unavailable, return a clear error message with installation instructions. If a session file is corrupt, skip it with a warning and continue loading valid sessions.

4. **Atomic Persistence**: Session files are written to `.tmp` then renamed. This prevents partial writes from corrupting session state on crash or timeout.

5. **Node.js ESM Hooks**: Hook scripts are Node.js ESM modules (`.mjs`). They read event JSON from stdin, write `hookSpecificOutput` JSON to stdout when active, and exit 0 on no-op. All hooks fail-open: any unhandled error exits silently to avoid blocking Claude Code.

## Source Tree Policy

| Directory | Layer | Contents | Modification Rule |
|-----------|-------|----------|-------------------|
| `src/bridge/` | L2 | MCP stdio proxy to backend daemon | Thin relay — changes affect MCP transport, not execution logic |
| `src/cli/` | L2 | Commander CLI client (parallel Bash-tool client) | CLI wiring — changes affect CLI subcommands, not backend logic |
| `src/execution/` | L1 | Persistent HTTP backend daemon (lifecycle, routing, service, engine, sessions) | Backend core — changes affect all providers and job lifecycle |
| `src/execution/discuss/` | L1 | Discuss runtime (operations, loop, subflows, persistence, registry) | Discuss runtime — imperative shell around the discuss domain core |
| `src/providers/` | L0 | Provider adapter system (registry, bootstrap, types) | Adapter framework — changes affect provider discovery and wiring |
| `src/providers/codex/` | L0 | Codex adapter (schemas, executor, parser, detection) | Provider adapter — preserve Codex wire-compatibility |
| `src/providers/claude/` | L0 | Claude adapter (schemas, executor, detection) | Provider adapter — preserve Claude JSON + resume semantics |
| `src/discuss/` | L0 | Domain core: state machine, reducer, events, projections, schemas | Functional core — pure state functions, zero I/O |
| `src/kb/` | L0 | Knowledge base: search, mutation, curation, Orama indexing | KB domain — changes affect search ranking and note lifecycle |
| `src/workflow/` | L0 | Pipeline executor: parser, AST, launch/retry/wait | Workflow engine — dependency-injected via ExecutionService |
| `src/shared/` | L0 | Shared utilities (mcp-utils, schemas, types, SSE parser) | Changes affect bridge, execution, and cli consumers |
| `src/infra/` | L0 | Path resolution, backend connection info | Infrastructure — changes affect all layers |
| `src/client/` | L0 | Public barrel for external consumers (readers, discuss DTOs, lifecycle) | Public API surface — changes affect coral-reef and CLI |
| `agents/` | — | Agent markdown definitions (Claude-native + Codex delegation + discuss) | Follow agent template structure |
| `skills/` | — | SKILL.md files for slash commands | Frontmatter must match plugin.json |
| `hooks/` | — | Hook scripts (`.mjs`) + hooks.json config | Node.js ESM, timeout-safe, fail-open |
| `bridge/` | — | Bundled output (coral-ax.cjs, coral-backend.cjs, coral-cli.cjs) | Generated by build — do not edit directly |
| `docs/` | — | Architecture, module docs, guides | Update when architecture or API surface changes. Do not duplicate source artifacts. |

Key rules:
1. Layer dependency: code in Lx may only depend on L0..L(x-1)
2. L0 modules never import from L1 (`execution/`) or L2 (`bridge/`, `cli/`)
3. `src/client/` straddles L0/L1 with type-only imports from execution — layering holds at runtime

## Module Structure

Dependency direction is strict: `bridge/server.ts` is the MCP stdio proxy composition root (thin relay to backend); `execution/server.ts` is the backend daemon composition root (HTTP server, tool routing, job lifecycle); `cli/bootstrap.ts` is the CLI client entry point. Lower modules never import from upper modules. See `docs/architecture.md` for the current dependency graph.

The discuss system follows a **Functional Core / Imperative Shell** pattern: `src/discuss/state-machine.ts` and `src/discuss/reducer.ts` contain pure state transitions with zero I/O; `src/execution/discuss/` handles all runtime I/O, persistence, and loop control.
