...
## Rule
The Glob tool fails silently (0 matches) when `path` points to `~/.claude/plugins/cache/coral/`, regardless of pattern or whether `~` is expanded. As of Claude Code ~2.2+, this path is inaccessible to Glob. Use `Bash("ls ~/.claude/plugins/cache/coral/coral/*/methods/")` instead — shell glob `*` resolves the version directory dynamically.
...
Bash("ls ~/.claude/plugins/cache/coral/coral/*/methods/")
