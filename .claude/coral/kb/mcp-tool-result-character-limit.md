...
## Rule
Claude Code enforces a character limit on MCP tool results (~40–50 K chars, not tokens). When gating inline content embedding, always measure the fully serialized `CallToolResult` body — `JSON.stringify(jsonResult(payload)).length` — not raw content length or plain `JSON.stringify(payload).length`. Use `MAX_INLINE = 10_000` as the safe threshold with headroom below the hard limit.
## Why
...
Right:
```typescript
const MAX_INLINE = 10_000; // in src/shared/schemas.ts
