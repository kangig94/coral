...
## Rule
If the stdio proxy is meant to become a pure MCP↔HTTP bridge and providers are meant to be pluggable via the registry, `src/backend/tool-router.ts` must not contain provider-specific execution normalization. Backend routing may select a provider from the registry and invoke shared built-ins, but defaults like launch `work_dir` or op-shape quirks must live in provider adapters or a provider contract that all adapters implement.
## Why
...
if (rawOp === 'exec' && !hasSession) {
  return { ...args, work_dir: projectRoot };
}
```
