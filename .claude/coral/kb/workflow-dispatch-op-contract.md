...
## Rule
When a workflow executor dispatches nested AX tool calls through `handleToolCall`, it must build provider-compatible payloads including an explicit `op` field (`coral:<agent>` for agent atoms) rather than forwarding only shared execution fields. `model`, `working_directory`, `effort`, and `bypass` are insufficient without the operation discriminator.
## Why
