...
## Rule
When calling `discuss({ op: 'bid', agent_name: ... })` or `discuss({ op: 'speak', agent_name: ... })`, the `agent_name` field must be the exact lowercase registered name (e.g., `"Klaus"` → `"klaus"`), not the persona display name (e.g., `"Klaus Becker"` or `"박지수"`). The `agent_name` field validates against `identPattern` (`/^[a-zA-Z0-9][a-zA-Z0-9._-]*/`), which rejects spaces and non-ASCII characters.
