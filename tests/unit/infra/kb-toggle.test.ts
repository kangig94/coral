...
import { CORAL_KB_ENABLED_ENV, KB_DISABLED_REASON, resolveKbEnabled } from '#src/infra/kb-toggle.js';
...
describe('resolveKbEnabled', () => {
  ...
  it('exposes stable env-key and disabled-reason constants', () => {
    expect(CORAL_KB_ENABLED_ENV).toBe('CORAL_KB_ENABLED');
    ...
    expect(KB_DISABLED_REASON).toBe('disabled (CORAL_KB_ENABLED=0)');
  });
});
