...
describe('migrations idempotency', () => {
  ...
  it('schema.sql and the applied migrations yield matching schema state', () => {
    ...
    try {
      ...
      const objectsA = (
        dbFromSchema.prepare("SELECT name FROM sqlite_master WHERE type IN ('table','index') ORDER BY name").all() as {
          name: string;
        }[]
      ).map((row) => row.name);
      const objectsB = (
      ...
      expect(metaA.map((row) => row.key)).toEqual([
        'coordinator_id',
        'created_ts',
        'journal_version',
        'schema_version',
      ...
      const corpusStateA = dbFromSchema
        .prepare(
      ...
      const corpusStateB = dbFromMigration
        .prepare(
          'SELECT id, snapshot_id, content_seq, metadata_seq, content_manifest_hash, metadata_manifest_hash FROM kb_corpus_state',
        )
        .get() as {
      ...
      expect(activeClaimColumnsA).toEqual([
        'id',
        'through_seq',
        'through_entry_id',
        'through_entry_kind',
      ...
      expect(dbFromMigration.prepare('SELECT COUNT(*) AS count FROM kb_curate_active_claim').get()).toEqual({
        count: 0,
      });

      const fingerprintColumnsA = (
        dbFromSchema.prepare("PRAGMA table_info('kb_curate_community_summary_input_fingerprints')").all() as Array<{
          name: string;
        }>
      ).map((row) => row.name);
      const fingerprintColumnsB = (
        dbFromMigration.prepare("PRAGMA table_info('kb_curate_community_summary_input_fingerprints')").all() as Array<{
          name: string;
        }>
      ).map((row) => row.name);
      ...
      const discoveryBacklogNotesColumnsB = (
        dbFromMigration.prepare("PRAGMA table_info('kb_curate_discovery_backlog_notes')").all() as Array<{
          name: string;
        }>
      ).map((row) => row.name);
