...
describe('markError terminal export', () => {
  function runMarkError(ensureResultArtifact: (jobId: string) => string) {
    const order: string[] = [];
    const settleFault = vi.fn(() => {
      order.push('settle');
      return COORDINATOR_NOT_APPLICABLE_FACTS;
    });
    const disposition = applyRecoveryAction(
      { type: 'markError', jobId: JOB_ID, fault: { kind: 'ghost_launch' }, status: {} as never },
      {
        progressStore: {
          ensureResultArtifact: (jobId: string) => {
            order.push('export');
            return ensureResultArtifact(jobId);
          },
        } as never,
        ...
        runningRecoverable: [],
        log: vi.fn(),
        runtime: new SimulationRuntime() as never,
        createInvocationContext: () => ({}) as never,
        getRecoveryService: () => ({}) as never,
        signal: new AbortController().signal,
        settleFault,
        settleClaim: vi.fn(),
        setProcessLocalCleanup: vi.fn(),
    ...
    return { disposition, order };
  }

  it('renders the export only after the fault is durable', async () => {
    const seen: string[] = [];
    const { disposition, order } = runMarkError((jobId) => {
      seen.push(jobId);
      return `/exports/${jobId}/result.md`;
    });

    expect(await disposition).toMatchObject({ kind: 'advanced', outcome: 'settled', detail: 'recovery fault settled' });
    expect(seen).toEqual([JOB_ID]);
    expect(order).toEqual(['settle', 'export']);
  });

  it('settles the fault even when the export cannot be written', async () => {
    const { disposition, order } = runMarkError(() => {
