...
describe('ProviderOperationReconciler publication', () => {
  it('refuses every requested stop after admission closes without changing a saga row', () => {
    const harness = createHarness();
  ...
  it('answers no-operation when a requested job has no provider-operation row', () => {
    const harness = createHarness();

    expect(harness.reconciler.requestStops(['job-without-operation'], 'signal_abort')).toEqual({
      kind: 'answered',
      outcomes: new Map([['job-without-operation', { kind: 'no-operation' }]]),
    });
  });

  it('reports an unreadable saga stop as unrecorded', () => {
    const warnings: string[] = [];
    const harness = createHarness({ onError: (message) => warnings.push(message) });
    const record = providerOperationRecord('executing');
    const duplicate = providerOperationRecord('executing', {
      operation: {
        ...record.operation,
    ...
    insertProviderOperation(harness.db, record);
    insertProviderOperation(harness.db, duplicate);
    ...
    expect(harness.reconciler.requestStops([record.operation.jobId], 'signal_abort')).toEqual({
      ...
      outcomes: new Map([
        [
          ...
          {
            kind: 'unrecorded',
            reason: `Job '${record.operation.jobId}' has more than one live provider operation.`,
          },
        ],
      ]),
    });
    expect(warnings).toEqual([
      `Provider operation stop intent failed for job '${record.operation.jobId}': Job '${record.operation.jobId}' has more than one live provider operation.`,
    ]);
  });

  it('reports a failed saga stop compare-and-swap as unrecorded', () => {
    const warnings: string[] = [];
    const harness = createHarness({ onError: (message) => warnings.push(message) });
    const record = providerOperationRecord('executing');
    insertProviderOperation(harness.db, record);
    expect(harness.reconciler.requestStops(['rowless-job'], 'signal_abort').kind).toBe('answered');
    const getDb = vi.spyOn(harness.progressStore, 'getDb');
    let getDbCalls = 0;
    getDb.mockImplementation(() => {
      getDbCalls += 1;
      if (getDbCalls === 3) harness.reconciler.stop();
      return harness.db;
    });
    ...
    expect(harness.reconciler.requestStops([record.operation.jobId], 'signal_abort')).toEqual({
      kind: 'answered',
      outcomes: new Map([
        [record.operation.jobId, { kind: 'unrecorded', reason: 'Provider operation mutation admission is closed.' }],
      ]),
    });
    expect(warnings).toEqual([
      `Provider operation stop intent failed for job '${record.operation.jobId}': Provider operation mutation admission is closed.`,
    ]);
  });

  it('keeps a written saga stop recorded when its immediate registry effect throws', () => {
    const warnings: string[] = [];
    const harness = createHarness({ onError: (message) => warnings.push(message) });
    const record = providerOperationRecord('executing');
    insertProviderOperation(harness.db, record);
    harness.registry.stop.mockImplementationOnce(() => {
      throw new Error('registry stop failed after write');
    });
    ...
    expect(warnings).toEqual([
      `Provider operation stop follow-up failed for job '${record.operation.jobId}': registry stop failed after write`,
    ]);
  });

  it('reports a reconciliation failure after a written saga stop without changing its outcome', async () => {
    const warnings: string[] = [];
    const harness = createHarness({ onError: (message) => warnings.push(message) });
    const record = providerOperationRecord('executing');
    insertProviderOperation(harness.db, record);
    vi.spyOn(harness.reconciler, 'reconcile').mockRejectedValueOnce(new Error('reconcile failed after write'));
    ...
    await vi.waitFor(() =>
      expect(warnings).toEqual([
        `Provider operation stop follow-up failed for job '${record.operation.jobId}': reconcile failed after write`,
      ]),
    );
  });

  it('reports an unreadable saga row before returning admission-closed', () => {
    const warnings: string[] = [];
    const harness = createHarness({ onError: (message) => warnings.push(message) });
    const record = providerOperationRecord('executing');
    const duplicate = providerOperationRecord('executing', {
