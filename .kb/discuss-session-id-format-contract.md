...
## Why
If only generation/storage changes, newly created session IDs fail Zod validation on every subsequent discuss tool call. If only the regex changes to the new format, legacy sessions become unreachable. Both failure modes are high-impact because they break normal discussion flow after `discuss({ op: "create", ... })`.
## Pattern
```typescript
