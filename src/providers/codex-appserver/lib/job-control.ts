...
function getCurrentSessionId(options: { env?: NodeJS.ProcessEnv } = {}): string | null {
  return options.env?.[SESSION_ID_ENV] ?? process.env[SESSION_ID_ENV] ?? null;
}

function filterJobsForCurrentSession(jobs: JobRecord[], options: { env?: NodeJS.ProcessEnv } = {}): JobRecord[] {
...
function formatElapsedDuration(startValue: string | undefined, endValue: string | null = null): string | null {
...
export function enrichJob(job: JobRecord, options: EnrichOptions = {}): EnrichedJob {
...
export function readStoredJob(workspaceRoot: string, jobId: string): unknown {
...
function matchJobReference(jobs: JobRecord[], reference: string | null | undefined, predicate: (job: JobRecord) => boolean = () => true): JobRecord | null {
...
export function buildSingleJobSnapshot(cwd: string, reference: string, options: EnrichOptions = {}): SingleJobSnapshot {
  const workspaceRoot = resolveWorkspaceRoot(cwd);
...
export function resolveResultJob(cwd: string, reference: string | null | undefined): ResolvedJob {
  const workspaceRoot = resolveWorkspaceRoot(cwd);
...
export function resolveCancelableJob(cwd: string, reference: string | null | undefined): ResolvedJob {
  const workspaceRoot = resolveWorkspaceRoot(cwd);
  const jobs = sortJobsNewestFirst(listJobs(workspaceRoot));
