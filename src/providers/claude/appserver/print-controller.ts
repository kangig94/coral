...
} from './print-control-protocol.js';
import {
...
export class PrintSessionController implements BrokerSessionController {
  private readonly spawnChild: PrintSessionControllerOptions['spawnChild'];
  private readonly onTurnStarted: PrintSessionControllerOptions['onTurnStarted'];
  private readonly stderrLimit: number;
  private readonly ids: PrintSessionControllerOptions['ids'];
  private readonly now: PrintSessionControllerOptions['now'];
  private readonly controlRequestTimer: PrintSessionControllerOptions['controlRequestTimer'];
  ...
  constructor(options: PrintSessionControllerOptions) {
    ...
    this.controlRequestTimer = options.controlRequestTimer;
    this.controlRequestTimeoutMs = options.controlRequestTimeoutMs ?? DEFAULT_CONTROL_REQUEST_TIMEOUT_MS;
  ...
  private async ensureInitializedSession(
    ...
    if (!this.childBinding) {
      ...
      this.defaultModel = params.model ?? this.defaultModel;
  ...
  private handleChildLine(line: string): void {
    const message = parseClaudePrintStdoutLine(line);
    if (message === null) {
      return;
    }
    ...
    if (message.response.subtype === 'error') {
      pending.reject(
        new ClaudeBrokerRpcError(
          CLAUDE_BROKER_STATE_RPC_CODE,
          message.response.error ?? 'Claude control request failed.',
          this.errorData({
  ...
  private handlePermissionRequest(message: SDKPermissionRequestMessage): void {
    ...
    if (!this.bootstrapSignature) {
      try {
        this.writePermissionResponse(message, 'deny');
      } catch (error) {
        const rpcError = this.asRpcError(error, 'Failed to deny Claude tool request.');
        if (turn) {
          this.failActiveTurn(turn, rpcError.message, 'internal-error');
        }
      }
      return;
    }

    if (!isAutoAllowPermissionMode(this.bootstrapSignature.permissionMode)) {
      if (turn) {
        this.emitTurnProgress(
          turn.brokerTurnId,
          `Claude requested ${message.request.tool_name} permission; denied because print transport cannot prompt interactively in ${this.bootstrapSignature.permissionMode} mode.`,
        );
      }
      try {
        this.writePermissionResponse(message, 'deny');
    ...
    try {
      this.writePermissionResponse(message, 'allow');
    } catch (error) {
  ...
  private writePermissionResponse(message: SDKPermissionRequestMessage, behavior: 'allow' | 'deny'): void {
    this.writeChild({
  ...
  private handleSystemMessage(message: SDKSystemMessage): void {
    if (message.subtype === 'init' && typeof message.model === 'string') {
      ...
      if (this.activeTurn) {
        this.activeTurn.model = message.model;
      }
    }
  ...
  private handleResultMessage(message: SDKResultMessage): void {
    ...
    const completed: ControllerNotificationMap['turn/completed'] = {
      ...
      model: typeof message.model === 'string' ? message.model : turn.model,
  ...
  private handleChildExit(event: ChildExit): void {
    ...
    if (turn) {
      this.activeTurn = null;
      this.lastTerminalTurnId = turn.brokerTurnId;
      this.emitTurnFailure(turn, exitError.message, 'child-exit');
    }
  ...
  private async sendControlRequest(request: ControlRequestPayload): Promise<unknown> {
    ...
    const cancelTimeout = (): void => {
      if (timeout !== null) {
        this.controlRequestTimer.cancel(timeout);
        timeout = null;
      }
    };
    const responsePromise = new Promise<unknown>((resolve, reject) => {
      ...
      if (this.controlRequestTimeoutMs > 0) {
        timeout = this.controlRequestTimer.schedule(() => {
          ...
          pending.reject(
            new ClaudeBrokerRpcError(
              CLAUDE_BROKER_STATE_RPC_CODE,
              `Claude control request ${request.subtype} timed out after ${this.controlRequestTimeoutMs}ms.`,
              this.errorData({
                pendingSubtype: request.subtype,
              }),
            ),
          );
  ...
  private emitTurnFailure(turn: ActiveTurnState, message: string, reason: TurnFailureDiagnosticReason): void {
    this.emitNotification({
      method: 'turn/failed',
      params: {
        brokerTurnId: turn.brokerTurnId,
        ...
        sessionId: this.latestSessionId,
        conversationRef: this.currentConversationRef(),
        diagnostic: this.buildTurnFailureDiagnostic(turn, reason),
      },
    });
  }

  private failActiveTurn(turn: ActiveTurnState, message: string, reason: TurnFailureDiagnosticReason): void {
    if (this.activeTurn !== turn) {
      return;
    }
    this.activeTurn = null;
    this.lastTerminalTurnId = turn.brokerTurnId;
    this.emitTurnFailure(turn, message, reason);
  }
  ...
  private buildTurnFailureDiagnostic(
    ...
    return {
      ...
      attempts: 1,
      childOutputTail: this.stderrRing,
      transcriptTail: '',
  ...
  private asRpcError(error: unknown, fallbackMessage: string): ClaudeBrokerRpcError {
    ...
    return new ClaudeBrokerRpcError(CLAUDE_BROKER_STATE_RPC_CODE, errorMessage(error) || fallbackMessage, {
      ...this.errorData(),
