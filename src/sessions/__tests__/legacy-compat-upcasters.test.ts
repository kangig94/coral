...
describe('sessions legacy compat upcasters (AC3.2, AC3.6)', () => {
  it('upcasts legacy session fault bodies into canonical session event shapes', () => {
    ...
    try {
      ...
      expect(decodeEventBody(readStoredEvent(db, 'session-legacy', 'session.interrupted', appended[1].seq)!.body)).toEqual({
        kind: 'app_server_interrupted',
        trigger: 'handoff',
        continuity: 'pre_checkpoint_preserved',
      });
      expect(decodeEventBody(readStoredEvent(db, 'session-legacy', 'session.adapter_unparseable', appended[2].seq)!.body)).toEqual({
        kind: 'adapter_output_unparseable',
        provider: 'claude',
        exitCode: 19,
        stdout: 'partial stdout',
      ...
      expect(decodeEventBody(readStoredEvent(db, 'session-legacy', 'session.provider_failed', appended[3].seq)!.body)).toEqual({
        kind: 'provider_session_unavailable',
        provider: 'claude',
        note: 'thread missing',
      });
      expect(decodeEventBody(readStoredEvent(db, 'session-legacy', 'session.provider_failed', appended[4].seq)!.body)).toEqual({
        kind: 'provider_request_failed',
        provider: 'codex',
        message: 'transport reset',
      });
