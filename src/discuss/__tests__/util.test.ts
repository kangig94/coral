 ...
 * Targets: util/string, util/rng, util/dpp, util/lock, handler-utils
 *
 ...
 * array lengths, sigma=0 kernel, k-DPP degenerate inputs, resultToMcp error
 * branch without detail, endContent exhaustive key coverage, lock timeout.
 */
...
import { resultToMcp, endContent, nowIsoString, resolveSession } from '../handler-utils.js';
...
describe('resultToMcp', () => {
  ...
  it('should return JSON with error field for error result', () => {
    ...
    const mcp = resultToMcp(result);
    expect(mcp.isError).toBe(false); // resultToMcp uses jsonResult, not textResult with isError
    const parsed = JSON.parse(mcp.content[0].text) as Record<string, unknown>;
  ...
  it('should handle error result with no detail field (detail is optional)', () => {
    ...
    // Must not throw even though result.detail is undefined
...
describe('resolveSession', () => {
  it('should return the error McpResult when session is not found', () => {
    // Minimal SessionStore mock — resolveDir returns null
    ...
    // Must return an McpResult (object with content array), not a string
