...
const JOBS_API_PATH = fileURLToPath(new URL('../../jobs/api.ts', import.meta.url));
const WORKFLOW_API_PATH = fileURLToPath(new URL('../../workflow/api.ts', import.meta.url));
const COORDINATOR_ROOT_PATH = fileURLToPath(new URL('../../coordinator/coordinator.ts', import.meta.url));
...
const ALLOWED_JOBS_API_EXPORT_TARGETS = new Set(['./startup.js']);
...
describe('coordinator api export scope invariant (AC12)', () => {
  ...
  it('keeps jobs/api.ts as a thin startup facade', () => {
    ...
    expect(
      collectExportTargets(sourceFile),
      'src/jobs/api.ts must only re-export the direct startup owner',
    ).toEqual(['./startup.js']);
  ...
  it('keeps workflow/api.ts as a thin facade over workflow owners', () => {
    ...
    expect(collectNonExportStatements(sourceFile), 'src/workflow/api.ts must contain only re-export declarations').toEqual([]);
  ...
  it('keeps jobs/api.ts and workflow/api.ts out of the production import graph', () => {
    ...
    const importingEdges = PRODUCTION_IMPORT_EDGES
      .filter(({ target }) => façadeTargets.has(target))
      .map(({ source, target, specifier, via }) => `${source} -> ${target} via ${via} (${specifier})`);
