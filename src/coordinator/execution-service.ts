...
type ExecIntent = Parameters<ProjectRequestPort['start']>[1];
type ResumeIntent = Parameters<ProjectRequestPort['resumeBySessionId']>[0];
type ForkIntent = Parameters<ProjectRequestPort['forkBySessionId']>[0];
...
export class ExecutionService implements RecoveryCapableService, ProjectRequestPort {
  ...
  private readonly providerHostManager: ExecutionServiceDeps['providerHostManager'];
  private readonly launchCoordinator: ExecutionServiceDeps['launchCoordinator'];
  ...
  private readonly readJobProgress?: (jobId: string) => JobProgress[];
  ...
  completeRecoveredJob(
    ...
    this.writeJobTerminal(jobId, sessionId, result, phase);
  ...
  private writeJobTerminal(jobId: string, sessionId: string, result: JobTerminal, phase: JobPhase): void {
    this.launchOrchestrator.writeJobTerminal(jobId, sessionId, result, phase);
  }
