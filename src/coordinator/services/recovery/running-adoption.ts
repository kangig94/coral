...
async function settleDurableContainment(
  ...
  const { signal, coordinatorCommit, abandonHeldJob } = settlement.context;
  ...
  if (
    ...
    if (cleanup.kind === 'absence-confirmed') {
        ...
        const retryHeldCleanup = async (): Promise<void> => {
          ...
          const retryCleanup =
            currentContainment.observation.kind === 'absent'
              ...
              : currentContainment.evidence.kind === 'current' || currentContainment.evidence.kind === 'provisional'
                ...
                : { kind: 'held' as const, reason: durableOwnershipEvidenceHoldReason(currentContainment.evidence) };
          ...
          if (retryCleanup.kind === 'held') {
            writeDurableCliContainmentStatus(progressStore.getDb(), {
              ...
              disposition: { kind: 'held', reason: retryCleanup.reason, retryIntervalMs, abandonment: 'abort-job' },
            });
            if (
      ...
      progressStore.appendProgress(jobId, launchRecord.sessionId, detail);
