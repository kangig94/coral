...
export function formatHandoffPublicationFailureSuccessor(
  outcome: PublicationFailure,
  context: 'observed-operation' | 'routing-resolution' = 'observed-operation',
): string {
  const retryTarget = context === 'routing-resolution' ? 'this resolve command' : 'the operation';
  switch (outcome.kind) {
    case 'not-published': {
        ...
        case 'invalid-record':
          return (
            ...
            (context === 'routing-resolution'
              ? 'rerun coral-cli backend routing-status resolve --invocation <id>.'
              : 'rerun coral-cli backend status.')
          );
        case 'rejected-transition':
          return `Next step: rerun coral-cli backend status and follow the successor shown for the invocation; do not assume ${context === 'routing-resolution' ? 'resolution' : 'publication'} occurred.`;
