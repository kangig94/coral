 ...
 *
 * Two flags are refused rather than resolved by precedence: each bootstrap capsule is one-use and consumed
 * by claiming its path, so a process that read one flag and ignored another would silently destroy the
 * credential of the role it did not become. A relative or non-canonical capsule path is refused up front,
 * before any capsule file is touched, matching `bootstrap-capsule.ts`'s own canonical-path requirement.
 */
export function parseProviderRoleArgv(argv: readonly string[]): ProviderRoleArgv {
  ...
  for (const [flag, role] of Object.entries(PROVIDER_ROLE_FLAGS)) {
    ...
    if (index !== -1) matches.push({ role, index });
  }
