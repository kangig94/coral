...
export class KbSourceImportService {
  ...
  start(
    ...
    if (request.async) {
      const { jobId } = this.shell.launchAsync('kb.source_import', jobCtx, (job) =>
        this.runImport(job, resolvedRequest, kbRuntime),
      );
      return kbSuccess({
        status: 'running',
        job: jobId,
    ...
    return this.shell.runSync('kb.source_import', jobCtx, (job) => this.runImport(job, resolvedRequest, kbRuntime));
  }
  ...
  private async runImport(
    ...
    const prepared = await prepareSourceImport(
      ...
      kbRuntime.kb.runtimeDir,
      this.deps.runtime,
      sourceImportOptions,
    );
    throwIfAborted(job.signal, 'persist');
    const persisted = await persistPreparedSource(kbRuntime.kb, prepared.stagedPath, prepared.slug, {
      signal: job.signal,
    });
    kbRuntime.curateScheduler.scheduleDeferredCommit();
    throwIfAborted(job.signal, 'readiness');
    const snapshot = kbRuntime.kb.getCorpusStateSnapshot();
