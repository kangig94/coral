...
gpt-6-sol  │ 5h: 0% (4:59) wk:22% (2.8d) │ spark 5h: 3% (0:47) wk: 1% (6.8d)
```
...
| `CORAL_CODEX_MODEL`           | `gpt-6-sol`         | Default Codex CLI model. On sized baselines (GPT-5.6 sizes or `gpt-6-astra`/`sol`/`luna`), abstract tiers map `fable`→`astra`, `opus`→`sol`, `sonnet`→`terra`, `haiku`→`luna`; bare aliases `astra`/`sol`/`terra`/`luna` are accepted; other baselines (e.g. `gpt-5.5`) use that single model for all abstract tiers |
| `CORAL_CODEX_EFFORT`          | `high`              | Codex reasoning effort (`low`…`ultra`). Ceilings: Astra/Sol/Terra `ultra`, Luna `max`, gpt-5.5 `xhigh`; Terra/Luna floor `xhigh`                                                                                                                                                                                     |
| `CORAL_CODEX_FAST`            | _(none)_            | Codex fast-mode toggle (`1` = fast, `0` = default/off); falls back to top-level `service_tier` (`default`, `fast`, or `flex`) in the selected `$CODEX_HOME/config.toml`                                                                                                                                              |
| `CORAL_CLAUDE_MODEL`          | _(none)_            | Default model for Coral-launched Claude sessions — `fable`/`opus`/`sonnet`/`haiku`, a context variant such as `opus[1m]`, or a full id like `claude-opus-4-8`; unset = Claude's own default. A per-request model wins; tier aliases are capped by `CORAL_CLAUDE_MODEL_CAP`                                           |
...
| `CORAL_CLAUDE_MODEL_CAP`      | `fable`             | Maximum Claude model tier (`fable` > `opus` > `sonnet` > `haiku`)                                                                                                                                                                                                                                                    |
...
| `CORAL_SYSTEM_PROVIDER_SCOPE` | _(none)_            | Named strict system profile scope for HTTP and daemon-internal provider work. Unset disables those launch paths; changing it requires a daemon restart ([details](docs/configuration.md#multi-account-provider-routing))                                                                                             |
