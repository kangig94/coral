...
| `CORAL_CODEX_MODEL`           | `gpt-5.6-sol`       | Default Codex CLI model. On sized baselines (GPT-5.6 sizes or `gpt-6-astra`), abstract tiers map `fable`→`astra`, `opus`→`sol`, `sonnet`→`terra`, `haiku`→`luna`; bare aliases `astra`/`sol`/`terra`/`luna` are accepted; other baselines (e.g. `gpt-5.5`) use that single model for all abstract tiers |
