...
| `CORAL_CLAUDE_MODEL_CAP`   | `opus`              | Maximum Claude model tier (`fable` > `opus` > `sonnet` > `haiku`)                                                                                                                                                                                                          |
...
| `CODEX_HOME`               | `~/.codex`          | Codex account selected for this invocation. Coral binds the resolved home to the session and uses the same account for resume/recovery ([details](docs/configuration.md))                                                                                                  |
| `CLAUDE_CONFIG_DIR`        | _(unset → ambient)_ | Claude account selected for this invocation. Use an explicit absolute directory for account isolation; ambient mode uses the OS user's default Claude/Keychain context ([details](docs/configuration.md))                                                           |
