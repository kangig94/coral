...
Start a structured discussion with AI agents. Pass `--user` to participate as a human observer.
...
2. **Analyze topic**: Determine team composition (roles, debate mode detection) and prepare persona inputs.
...
3. **Generate personas**: Call `discuss_lead({ op: '_1_seed', ... })` then spawn `persona-generator` agents in parallel (one per role, `model: "sonnet"`).
...
4. **Initialize**: Build the agents list. If `--user` flag was passed, add `{ name: 'user', persona: '# User — Human Participant\nHuman observer with real-time participation via /bid skill.', participation: 'observer' }` to the agents list. Call `discuss_lead({ op: "_2_create", ..., min_bid_delay_ms: 10000 })` if `--user`, else `min_bid_delay_ms: 0`. Get `session_id`.
...
9. **On discussion end**: discuss-lead calls `_7_end(synthesis)` → agents self-terminate via bid() cascade → discuss-lead sends synthesis to main context. Present the synthesis to the user. If `--user`, delete `.claude/coral/discuss/active-user-session.json`.
