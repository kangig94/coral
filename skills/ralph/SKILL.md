...
<Ralph_Protocol>
  ...
  <Codex_Mode>
    ...
    Execution loop:
    1) `codex({ op: "exec", ... })` → `{ job, session }`.
       `wait({ jobs: [job] })` → `result.text` contains the response. Pass `working_directory`.
    2) Keep using `session` for continuity: `codex({ op: "exec", session, ... })`.
