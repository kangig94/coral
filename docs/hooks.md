...
`wait jobs` maps terminal outcomes before considering siblings: failed, aborted, and faulted jobs map to `1`; `provider_exit` maps to its normalized child code; and `completed` maps to `0`. A non-zero mapped code returns immediately, whether or not siblings remain. A zero mapped code exits `0` when no requested siblings remain and `75` when they do, with a command for waiting on those siblings. Exit `75` is also a nonterminal waiting outcome when the CLI prints a resume command. A remediation command includes a cursor after the CLI has observed journal progress, but it is cursor-free if the initial `backend_recovering` or `backend_shutting_down` subscription attempts exhaust before any event is observed; follow the printed command exactly. A terminal block instead names any remaining job IDs directly. A terminal `provider_exit` whose child exited `75` also legitimately exits `75`; when no siblings remain it prints terminal output and no resume command. A nonterminal `interrupted` line may also appear, reporting that a job's carrier was observed absent while the job is still nonterminal: the subscription stays open and the exit code stays pending, so it is informational only, never one of the exit-code outcomes above. Only the journal ends a job. It is emitted today only for durable-CLI jobs, the one carrier class whose absence local evidence can establish.
