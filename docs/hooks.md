...
The `# Tools` section's `{{EQUIPPED_TOOLS}}` placeholder (right after the `CLI:` line) is filled from the daemon-written `equipped-tools.json` run-dir snapshot (`coordinatorPaths.equippedToolsFile`). `readEquippedToolsSnapshot()` reads it fail-open, and the block lists agent-facing tools installed via `/equip` with a directive to use them. The snapshot deliberately surfaces only externally-contributed install-only packages (e.g. `codebase-memory`); Coral's own engines and bundled install-only artifacts (e.g. the Kiwi tokenizer) are excluded as internal plumbing the agent never calls directly. This is a main-session-only surface: `subagent-start.mjs` and provider-launched sessions strip the placeholder. The coordinator refreshes the snapshot at boot and after every install-only install/uninstall (`writeEquippedToolsSnapshot`). Install/uninstall writes make the updated snapshot available to the next session immediately, while the boot refresh runs after the SessionStart hook has already read the snapshot for the current session, so it only affects later sessions.
