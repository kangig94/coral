...
5. **PostToolUseFailure** (`*`) - On any non-zero tool exit, reminds Claude to check `.kb/` before debugging
...
**Script: `hooks/kb-promote-gate.mjs`** — Checks for unprocessed memos in `$CLAUDE_PLUGIN_DATA/projects/<slug>/memo/`. Injects `hookSpecificOutput` with `additionalContext` reminding about KB promotion.
...
- **PostToolUseFailure** (`*`) — On any tool failure, immediately reminds Claude to check `.kb/`.
