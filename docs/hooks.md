...
- records a mode-`0600` write-ahead durability marker outside the repository before publishing an ignore artifact or symlink. The maintenance lock is also exactly `0600`, and Coral-owned state and repository-arena directories are exactly `0700`: creation normalizes the requested mode after the umask has acted, while a later encounter repairs an interrupted creation before use. A newly created user-owned ignore artifact instead preserves the umask-shaped group and other bits and adds only owner read/write. The marker filename is the SHA-256 of the absolute target stored in its content; reconciliation quarantines evidence when those two identities do not agree and never syncs the unverified target. Once a marker has been opened, an `fstat` or read failure retains it as `durability-evidence-unreadable` instead of treating it as absent. Clearing that hold requires marker readability and ownership to be repaired, or the filesystem or storage read failure to be fixed.
- reports arena cleanup and pending-marker reconciliation as independent artifacts. `durabilityReconciliation` is `reconciled` only when no marker hold remains; otherwise it is `refused` with every distinct reason in deterministic order. One shared renderer emits one sentence per distinct reason in deterministic order for both SessionStart and the direct project-ignore entrypoint; the latter keeps stdout as result JSON alone and writes the sentences to stderr. A discharged or quarantined marker therefore cannot hide a later retained marker that still needs action on either surface.
