...
- observes `.claude/` before inspecting `.claude/coral`, whether or not symlink creation is requested. When symlink creation is not requested, an observed missing or non-directory parent proves that no scoped Coral link or ignore file can exist, so the legacy sweep and Git-root retraction still run. When creation is requested, a non-directory parent is a structural refusal and a missing parent is a named refusal. A failure to observe the parent is always retryable. Coral never creates the working-tree parent. With a real parent, inspection distinguishes a missing `.claude/coral`, an observed link, an observed non-link, and a failure to observe the path. Only the observed non-link is a conflict; an observation failure refuses retryably and asks the operator to make the path observable. Preflight also prepares the target from `~/.coral` through the selected `~/.coral/projects` or `~/.coral/projects-dev` root and its project leaf. A symlink, non-directory, or containment conflict in that range refuses the symlink before Coral changes `.git/info/exclude`; an operational filesystem failure is retryable. Once preflight succeeds, Coral publishes the exclude entry, creates or repoints `.claude/coral -> ~/.coral/projects/{slug}/`, and only then retracts the legacy working-tree entries.
- records a mode-`0600` write-ahead durability marker outside the repository before publishing an ignore artifact or symlink. The maintenance lock is also exactly `0600`. Every Coral-owned directory used by this maintenance is exactly `0700`, including the `~/.coral` state and projects roots, each project leaf, staging and quarantine directories, transient run directories, and repository-arena components: creation normalizes the requested mode after the umask has acted, while a later encounter repairs an interrupted creation before use. Before reading `.git/info/exclude`, preflight refuses an existing `.git/info` that is a symlink, is not a directory, lacks owner read, write, and execute access, or has an access condition; a transient failure to observe it is the retryable `artifact-observation-failed` refusal, and the exclude itself is not observed or published. The operator must replace an invalid shape with a real directory or repair the owner access. A `.git/info` directory created by the current run and a newly created user-owned ignore artifact instead preserve the umask-shaped group and other bits while adding the owner access each artifact needs. Coral does not widen permissions on an existing `.git/info`; if an interrupted run left a newly created directory unusable, the operator must grant owner access or remove it before retrying. Initial ignore-file reads likewise distinguish a non-regular shape or access condition (`artifact-unreadable`) from a transient observation failure (`artifact-observation-failed`). `ENOENT` proves absence only while `lstat` or `open` is resolving the pathname; once `open` returns a descriptor, every thrown `fstat` or descriptor-read observation is `artifact-observation-failed`. After staging an update, Coral distinguishes an observed disappearance or content mismatch (`artifact-changed`) from a failed re-read (`artifact-observation-failed`); only the mismatch attributes the refusal to another change. Reconciliation enumerates the stable arena and decodes marker contents, independent of current artifact demand or a digest for any path held by the run. A valid marker is synced and removed only when its decoded target is inside the resolved project directory or common Git directory. A valid marker outside both is left untouched and unreported by this run so a run of its owning project can discharge it; if that project is never opened again, the marker remains in `~/.coral`, where it refuses and blocks nothing. The marker filename is the SHA-256 of the absolute target stored in its content; reconciliation quarantines evidence when those two identities do not agree and never syncs the unverified target. Once a marker has been opened, an `fstat` or read failure retains it as `durability-evidence-unreadable` instead of treating it as absent. Clearing that hold requires the arena and marker readability and ownership to be repaired, or the filesystem or storage read failure to be fixed.
- reports arena cleanup and pending-marker reconciliation as independent artifacts. A failed repository-arena preparation blocks an artifact only when that artifact needs a staged replacement or symlink repoint; if every artifact is already a no-op, the run remains `complete`. A symlink or non-directory arena component is a non-retryable structural conflict carrying the bounded component identity (`coral`, `staging`, or `project-ignore`), and its remedy expands that identity to the exact `<commonGitDir>/coral`, `<commonGitDir>/coral/staging`, or `<commonGitDir>/coral/staging/project-ignore` path. Observation and removal failures remain retryable. `durabilityReconciliation` is `reconciled` only when no marker hold remains; otherwise it is `refused` with every distinct reason in deterministic order. An artifact's `durability` likewise has one dominant `state` and a distinct, sorted `reasons` array, which is empty only for `synced`; a refused publication carries only a real failed or unsupported durability disposition, never a synced claim. An unsupported parent sync during publication retains a marker for the next run, while the corresponding reconciliation disposition reports that the marker was discharged and will not be retried. Failure to remove a final marker is retained on that refused artifact as `durability-evidence-cleanup-failed`. A failed marker write reports its retained staging file as `owned-staging`, and the final result clears that residue only after removing the run directory that owns it. Arena sweep and run-directory removal failures remain visible through `arenaSweep`, but that background cleanup artifact does not select the aggregate status; retained `owned-staging` on this run's affected artifact does. The legacy working-tree sweep likewise treats a failed directory enumeration, candidate observation, or symlink-authorship read as `legacy-sweep-observation-failed`, preserves its completed deletion count, and stops publication. One shared renderer produces one sentence per distinct reason in deterministic order for SessionStart and the owner/child result emitter; path-specific legacy remedies include the path carried by the artifact. The emitter writes the sentences to stderr before writing the result JSON alone to stdout. No durability problem on one artifact, and no discharged or quarantined marker before another retained marker, can hide a hold on either surface.
- runs ignore maintenance in a time-bounded child process and reports `complete`, `refused`, and `partial` results distinctly. An unavailable owner wrapper—including missing or non-executable `flock`—is reported with the lock remedy on both SessionStart and init-project paths. Each refusal renders its own remedy; only retryable reasons promise another attempt at the next SessionStart, while persistent device-topology refusals require the named manual repair. A partial pass either made progress or retained Coral-owned staging that cleanup could not remove, and is not reported like a pass that made no progress and retained no staging.
