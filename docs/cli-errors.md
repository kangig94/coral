...
| `75` | No settled verdict / do not proceed: `transient`, `backend_shutting_down`, `kb_disabled`, `kb_initializing`, `kb_offline`, `provider_host_inventory_unavailable` (matched by code name because the IPC path carries no HTTP status), and `coordinator_unreachable`/`coordinator_record_unreadable`/`coordinator_socket_dir_unverified` (matched via `NOT_OBSERVED_CORAL_SETUP_ERROR_CODES` in `src/runtime/errors.ts` — a shared list `errorCodeToExit` and `coral-cli expansion`'s own exported `expansionExitCode` both consult), plus generic HTTP `503` fallback (`errorCodeToExit` in `src/cli/errors.ts`). Retry only when that code's remediation says the condition can clear without intervention |
