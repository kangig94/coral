...
| `coordinator_socket_dir_insecure`             | The socket path beside the run directory exceeds this platform's `sun_path` limit, so Coral relocated it into a directory shared with every user on the host, and observed that the relocated path is not one it can keep private to this user: `reason` is `foreign` (the path belongs to another user, so only that owner or an administrator can clear it), `unusable` (the path is this user's, and is not a directory), or `unsecurable` (the parent permits other users to replace the entry, or the filesystem does not keep the mode Coral set). A loose mode on a directory this user owns is tightened rather than refused. Refused rather than bound, because the socket is the singleton coordinator lock. Raised on the coordinator's own startup path and on the offline operator commands that take the same socket as a guard, which pass it through rather than re-wrapping it as `coordinator_socket_bind_failed` |
