...
Provider-host `list` and `inspect` can report `reclamation-failed`. This status means a recorded process group was not confirmed reclaimed, not that the host was classified broken. Its host metadata names the recorded `pid`, `processGroupId`, `reclamationAttempts`, and `reclamationFailure`. Running `provider-host evict <ref>` against that exact row joins an in-flight automatic retry or retries reclamation after automatic attempts have stopped; the row clears only after absence is confirmed.
