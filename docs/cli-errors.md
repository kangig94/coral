...
| `coordinator_socket_dir_unverified`           | The same relocation, but creating, observing, or tightening the directory failed outright, so this run reached no conclusion about who owns it — exit 75, on the same "not observed" axis as `coordinator_unreachable` and `coordinator_record_unreadable`, rather than the exit `1` of the ownership verdict above |
