...
| `coordinator_socket_dir_insecure`             | The socket path beside the run directory exceeds this platform's `sun_path` limit, so Coral relocated it into a directory shared with every user on the host — and that directory is not one this user owns at mode `0700`. Refused rather than bound, because the socket is the singleton coordinator lock. Raised on the coordinator's own startup path; an offline operator command surfaces the same condition as `coordinator_socket_bind_failed` with this message as its cause |
