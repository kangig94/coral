...
| `provider_host_inventory_unavailable`         | A captured owner failed to return a valid complete inventory, or the selected owner's exact inspect/evict call failed after inventory capture. No partial rows or mutation are returned. Retry the original command; if it persists, run `coral-cli backend status` and restore the unavailable owner before retrying                                                                                                                                             |
| `provider_host_not_found`                     | No live or retained-blocked provider host matches the exact reference or canonical work-directory selector. Rerun `coral-cli backend provider-host list`, then use a currently listed reference                                                                                                                                                                                                                                                               |
...
| `provider_host_identity_integrity`            | Coral found duplicate owner IDs before selecting a host, or one exact host reference collided across records/owners. Do **not** evict: preserve the complete error output and escalate the integrity failure                                                                                                                                                                                                                                                    |
...
| `provider_host_unserviceable`                 | A failed provider job retained one exact blocked host reference. Copy the visible `ph1.…` reference, run `coral-cli backend provider-host inspect <ref>`, then run `coral-cli backend provider-host evict <ref>` before retrying fresh placement. Eviction may end work already attached to that host                                                                                                                                                              |
