...
| `1`  | User-correctable backend/domain errors such as `invalid_request`, `not_found`, `session_not_found`, `audit_requires_ended_session`, `scope_mismatch`, `unauthorized`, provider-host errors other than `provider_host_inventory_unavailable`, every generation-boundary/operator code in the table below, `store_reset_incident_not_found`, `store_reset_incident_limit_exceeded`, `coordinator_record_unreadable` (observed and durable — retry re-reads the same unreadable file and cannot clear it), and default `backend_error` cases |
| `75` | Retry-later, not-observed failures: `transient`, `backend_shutting_down`, `kb_disabled`, `kb_initializing`, `kb_offline`, `provider_host_inventory_unavailable` (matched by code name because the IPC path carries no HTTP status), and `coordinator_unreachable` (matched via `NOT_OBSERVED_CORAL_SETUP_ERROR_CODES` in `src/runtime/errors.ts` — currently that set's sole member — a shared list `errorCodeToExit` and `coral-cli expansion`'s own exported `expansionExitCode` both consult), plus generic HTTP `503` fallback (`errorCodeToExit` in `src/cli/errors.ts`)                                                                                                          |
...
`coral-cli backend shutdown` also exits `75`, and for the reason the code exists rather than a reuse of it: when the discovery record could not be read (`unreadable_record`), the connection was refused (`socket_refused`), a response arrived but did not resolve the question either way (`refused_by_response`), or the request never completed (`no_response`), this run did not observe whether a coordinator stopped. Its other refusals exit `1`, and are not all the same shape: `no_record` and `recorded_process_absent` *did* observe an absence, `capability_rejected` observed a coordinator that answered and declined — but `nested_child` observed nothing at all, because a nested Coral child refuses before asking anything, since it must never drain the coordinator its own parent job runs on (see [Nested Coral Commands](#nested-coral-commands)). `socket_refused` sits with the not-observed group rather than with the absences despite superficially looking like one, because the recorded pid was never established absent before the request was sent — an absent pid short-circuits to `recorded_process_absent` first — so a refused connection here only proves nothing answered at that exact socket at that moment; a coordinator's HTTP listener can close at the top of its drain while its process, already confirmed alive, keeps running through IPC close and store finalizers. The distinction is load-bearing because [Configuration](./configuration.md) tells operators to shut the backend down before `store-reset discard` and `kb-commit quarantine`, but exit `1` does not uniformly answer "you may proceed" — it means only that this run observed enough to say why it did not stop, or in the `nested_child` case, refused before observing anything. Proceeding is safe when the reason is a confirmed absence (`no_record`, `recorded_process_absent`); it is the opposite of safe when the reason is `capability_rejected`, a coordinator this run confirmed is still live and said no; and `nested_child` answers neither, because this run is not positioned to answer at all. Exit `75` answers "do not proceed, and this is not a failure — retry". A caller running this under `set -e` should also note that every exit-`1` case aborts the script alike, including the confirmed-absent ones where proceeding was actually fine — a non-zero exit stops the script before anything reads which case it was. The full mapping is `SHUTDOWN_REFUSAL_EXIT_CODES` (`src/cli/commands/backend.ts`), which is exhaustive over `ShutdownReason` so a new refusal cannot inherit either answer by default.

`coral-cli backend status` sets an exit code too, for the same reason `backend shutdown` does: `backend status && <destructive op>` used to read every outcome, including "state is unknown," as permission to proceed. It exits `0` for `ok`, `not_running`, `shutting_down`, `unauthorized`, and `recent_failure` — every status `getBackendStatusFull` reached a confident answer for — and `75` for `undecodable_record` and `unreachable`, the two statuses that function itself refuses to call either `ok` or `not_running` because it could not tell. The full mapping is `BACKEND_STATUS_EXIT_CODES` (`src/cli/commands/backend.ts`).
