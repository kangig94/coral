...
| `75` | Retry-later failures: `transient`, `backend_shutting_down`, `kb_disabled`, `kb_initializing`, `kb_offline`, and `provider_host_inventory_unavailable` (matched by code name because the IPC path carries no HTTP status), plus generic HTTP `503` fallback (`errorCodeToExit` in `src/cli/errors.ts`)                                                                                                          |
...
`coral-cli backend shutdown` also exits `75`, and for the reason the code exists rather than a reuse of it: when the discovery record could not be read (`unreadable_record`) or the shutdown request never completed (`unreachable`), this run did not observe whether a coordinator stopped. Its other refusals exit `1`, which means the opposite — the run *did* observe, and a coordinator is either absent or alive and unwilling (`capability_rejected`: it answered and declined). The distinction is load-bearing because [Configuration](./configuration.md) tells operators to shut the backend down before `store-reset discard` and `kb-commit quarantine`, but exit `1` does not uniformly answer "you may proceed" — it means only that this run observed enough to say why it did not stop. Proceeding is safe when that reason is a confirmed absence; it is the opposite of safe when the reason is `capability_rejected`, a coordinator this run confirmed is still live and said no. Exit `75` answers "do not proceed, and this is not a failure — retry". A caller running this under `set -e` should also note that both exit-`1` cases abort the script alike, including the confirmed-absent one where proceeding was actually fine — a non-zero exit stops the script before anything reads which case it was. The full mapping is `SHUTDOWN_REFUSAL_EXIT_CODES` (`src/cli/commands/backend.ts`), which is exhaustive over `ShutdownReason` so a new refusal cannot inherit either answer by default.
