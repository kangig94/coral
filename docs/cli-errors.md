...
An interrupted live wait also exits `75` with code `transient` when its delegated child ends from a signal, its subscription exhausts transient retries, or a reconnect exhausts transient open attempts after an earlier subscription opened. The error keeps the signal or subscription diagnostic as its message and adds an exact `coral-cli wait jobs <remaining-job-ids> --cursor <current-cursor>` remediation command. If no subscription opened, exhausted `backend_unreachable` attempts remain code `backend_unreachable` with exit `69`; there is no observed wait position to resume. A second locally received Ctrl+C remains different: it aborts the jobs and exits `1`; when the first local Ctrl+C is reflected back as delegated `SIGINT`, the CLI reconnects instead of emitting an error.
