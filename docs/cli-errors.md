...
`coral-cli backend shutdown` also exits `75`, and for the reason the code exists rather than a reuse of it: when the discovery record could not be read (`unreadable_record`) or the shutdown request never completed (`unreachable`), this run did not observe whether a coordinator stopped. Its other refusals exit `1`, which means the opposite — the run *did* observe, and a coordinator is either absent or alive and unwilling. The distinction is load-bearing because [Configuration](./configuration.md) tells operators to shut the backend down before `store-reset discard` and `kb-commit quarantine`: exit `1` answers "you may proceed", exit `75` answers "do not proceed, and this is not a failure — retry". The full mapping is `SHUTDOWN_REFUSAL_EXIT_CODES` (`src/cli/commands/backend.ts`), which is exhaustive over `ShutdownReason` so a new refusal cannot inherit either answer by default.
