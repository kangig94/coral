...
| `CORAL_KB_ENABLE`                 | _(unset → enabled)_                            | Set `0` to boot the daemon without spawning the KB child — no corpus indexing, curate, retrieval, or KB content injected into sessions/agents. `1` or unset enables it; a malformed value warns once and leaves KB enabled. Read from the daemon's environment at startup like `CORAL_KB_IMPORT_MAX_BYTES`. Flipping `0`→`1` and running any `kb …` command transparently restarts the daemon to bring KB online (that one command waits for daemon-ready; KB child boot remains non-blocking)                                      |
...
export CORAL_KB_ENABLE=0   # boot the daemon without the KB child; set to 1 (or unset) to re-enable
```
