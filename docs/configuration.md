...
| `CORAL_PROVIDER_PROXY_ORPHAN_TIMEOUT_MS` | `37000`                                        | How long a provider-proxy guardian or reaper may go without round-trip evidence from live control before it tears down its recorded process containment, in ms. The stated range is 19001..300000, but configuration also derives a floor from the timing budgets it must cover — teardown reserve, control lease, and successor tail — which currently raises the accepted minimum to 36001; a value in between passes the range check and is still rejected. That floor is computed, not written down twice, so it moves whenever those budgets move. Guardian and reaper read it when their role state machines start; the proxy reads no deadline configuration of its own. Fresh acquisition installs one exact-set recovery credential across all three roles and durably writes its protected capsule before publishing the set, so both graceful control release and abrupt coordinator loss use the same recovery path. Each operation joins succession membership before prepare. A successor must redeem the set before this deadline; otherwise the enforcers move it toward exact containment disappearance. |
