...
Provider jobs must match their owning binding. In the implementation through B03, a workflow's complete caller scope is attached to its durable workflow job launch, discussion state persists its complete caller scope directly, and each system one-shot binds the configured system scope at the point of use. Aggregate-root ownership of workflow/discussion scopes belongs to B04 and is not claimed by the B03 runtime. Claude's broker remains account-neutral while its controller receives either the explicit bound config directory or the exact home that owns the default `.claude`; Codex app-server identity includes the bound home. Codex account bindings require ChatGPT auth, pin thread start/resume/recovery to the official OpenAI model provider, and validate Codex's effective config before a thread operation; alternate credential stores, model providers, base URLs, remote thread config, and config lockfiles fail closed. Caller-supplied API keys, auth tokens, provider base URLs, and alternate cloud selectors likewise cannot override either provider's selected profile.
