...
- `store-reset discard` is the explicit offline path for a newer-store case. When the active-store selection names a valid newer local build, it reports a handoff and resets nothing; otherwise it publishes the same V3 `newer-incompatible-invalid-target` incident that automatic startup creates. Ordinary boot also creates incidents automatically for older or corrupt/unsupported stores. `current` maps to the internal `gen2` generation; `legacy` is inspection-only and a discard request always refuses before path resolution or socket binding.
