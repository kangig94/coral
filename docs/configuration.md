...
| `CODEX_HOME`                          | `~/.codex`                                     | Codex credential home selected at the invocation boundary. Coral canonicalizes it, binds it to the provider session, and routes preflight, app-server launch, resume, recovery, and artifact cleanup through that same source                                                                                                                                                                                                                                                                                                                                                       |
| `CLAUDE_CONFIG_DIR`                   | _(unset → ambient)_                            | Claude credential/config directory selected at the invocation boundary. An explicit value is used by preflight and the selected Claude controller, but never becomes broker/daemon identity. When absent, Coral does not set the variable and preserves Claude's ambient behavior; a canonical locator is retained for source-aware artifact lookup                                                                                                                                                                                                                              |
...
Coral runs one account-neutral daemon and store per flavor. Provider account selectors do not choose a socket, daemon, or state directory. Instead, every local IPC invocation carries a canonical credential-source set: `CODEX_HOME` selects Codex, while explicit `CLAUDE_CONFIG_DIR` selects Claude. A missing `CLAUDE_CONFIG_DIR` is represented as ambient authority and never inherits the account that happened to boot the daemon. HTTP requests use the daemon's captured defaults because remote callers cannot supply local filesystem credential paths.

Ambient Claude is compatibility behavior, not an isolated multi-account selector: it uses the daemon OS user's default Claude/Keychain context. Set an explicit absolute `CLAUDE_CONFIG_DIR` for every Claude account that must remain isolated.

The selected source becomes immutable session authority. Provider jobs must match that source; workflow and discuss state persist the complete source set so later steps, restart recovery, resume, and artifact cleanup cannot drift to whichever account invokes them next. Claude's broker remains account-neutral and its controller receives the selected config directory. Codex app-server identity includes the selected home. Caller-supplied credential env such as API keys is stripped, and unsupported Claude Bedrock, Vertex, or Foundry selectors fail closed.

Selectors must be absolute paths. Shell expansion happens before Coral runs, so use an expanded absolute path; a literal `~` or a relative path is rejected.
...
Resuming a provider session under a different selector fails with `provider_credential_source_mismatch` before preflight or queue admission. Re-run with the original `CODEX_HOME`/`CLAUDE_CONFIG_DIR`, or start a new session for the new account. A missing or unreadable selected source fails with `provider_credential_source_missing` or `provider_credential_source_unavailable`; authenticate that exact source and retry. For example, use `CODEX_HOME=/absolute/account/home codex login` or `CLAUDE_CONFIG_DIR=/absolute/account/config claude auth login`. For ambient Claude, leave `CLAUDE_CONFIG_DIR` unset while logging in.

The source record's `version: 1` is the strict shape version of the persisted credential descriptor, not a database migration switch. Event `bodyVersion` remains `1`; discuss snapshots remain strict `schemaVersion: 3`. Incompatible stores are reset, so this release promises no older event-body upcast or snapshot conversion/default fallback. The fresh store's `meta.journal_version=2` is an epoch label included in `schema.sql`, not a second migration switch. The database itself is governed by Coral's existing full-`schema.sql` hash in `PRAGMA user_version`. On the first backend start after this schema epoch changes, Coral moves the canonical DB/WAL/SHM into `store-reset-quarantine/<timestamp>-.../` and creates a fresh store. All Journal and projection state is lost, including live and completed jobs, sessions, workflows, and discussions; filesystem exports may remain, and the KB markdown corpus is unaffected. Retired `~/.coral/by-config/*` trees are ignored and are neither adopted nor deleted.
