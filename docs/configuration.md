...
| `CORAL_PROVIDER_PROXY_ORPHAN_TIMEOUT_MS` | `30000` | How long a provider-proxy guardian or reaper may go without round-trip evidence from live control before it tears down its recorded process containment, in ms. The stated range is 19001..300000, but configuration also enforces a redemption margin that raises the accepted floor to 24001 — a value in between passes the range check and is still rejected. Read by the guardian and reaper role processes' own entry point (`src/provider-proxy/role-main.ts`) whenever either role is dispatched, to build that role's deadline state machine; the proxy role reads no deadline configuration. Nothing else may name a value for it: the coordinator's set-acquisition steps refuse to install a handoff grant at all (`ProviderProxyHandoffGrantUnavailableError`) until the reaper half and the successor capsule exist, precisely so no second party can put a plausible-looking timeout into a grant binding that a successor would then read as agreed. Still has no observable effect today, because nothing spawns a set in production: the one function that would spawn a guardian (`createProviderProxyAcquisitionSteps`) has no caller. |
...
`CORAL_CLI_HANDOFF_DELEGATED` is another key you never set. When an older build's CLI finds a newer installed build, it delegates the invocation into that build and stamps this marker (`1`) on the child so the child cannot delegate again — a one-step guard against two builds handing off to each other. Its only legal values are `1`, `0`, and unset; anything else (including a declared-but-empty `export CORAL_CLI_HANDOFF_DELEGATED=`) is rejected as invalid usage naming the variable. If you ever see the refusal message, the daemon side is what to inspect (`coral-cli backend status`), not this variable.
...
`CORAL_KB_ENABLE` is re-read at daemon startup, so setting it while a daemon is already running takes effect only after a restart — and Coral will not force that restart for you. A `kb …` command run with `CORAL_KB_ENABLE=1` against a KB-disabled coordinator warns on stderr and then fails with `kb_disabled`; the running coordinator keeps serving, because no CLI invocation evicts a live one. Wait for the next idle restart (`CORAL_BACKEND_IDLE_MS`, default ~6h) or run `coral-cli backend shutdown` yourself when nothing else is in flight.
