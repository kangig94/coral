...
Reset authority and support inspection deliberately point in opposite directions. Only the backend lifecycle can mint `BackendStoreResetAuthority`; one settlement machine in the selection coordinator holds the reset lock, resumes interrupted work, mints one enumerate-classify-enumerate epoch, and publishes at most one incident. When the active name must become fresh, it mints a current-format store under quarantine and enters a counter-free claim loop. Each iteration writes a durable parking sidecar, parks all present names, classifies `store.db` only at its owned parked path, adopts a compatible occupant, or links the minted store into the vacant shared name. `EEXIST` starts the next epoch. One pathname owner mints `{ dev, ino }` identities and mediates every shared-name read, rename, and atomic link-back; callers receive no unlink capability. Linking avoids a size-keyed startup veto after writers drain; copy remains the unbounded fallback and records why linking was unavailable plus whether the copied snapshot was verified coherent or observed torn. The additive-tolerant ledger owns only the in-flight publication promise, preserved slot, aggregate descendant discards and over-bound preservation status, and validated stored Coral version. Terminal parked bytes are indexed by their own `parked.v1.json`, so list and release do not depend on a lossy ledger projection. `backend store-reset release` removes parking and durably syncs its parent before removing the canonical incident; an unproven sync is rendered on stderr with a transient exit. SQLite integrity work receives verified copies in a private temp directory, runs in a bounded child process with read-only/query-only pragmas, re-hashes the source evidence, and cannot import the backend reset boundary.
