...
Replacement creates and opens a complete current-format store under a private `.mint-<uuid>` directory, writes and durably syncs additive `epoch.json` metadata, and publishes the entire directory with one rename to `epoch-<N+1>`. The superseded epoch remains in place; no live SQLite pathname is renamed, unlinked, copied, or hashed. A concurrent publisher that finds a non-empty destination adopts the winner, and a publisher whose mint was swept remints. This makes the adoption lock a performance mechanism rather than correctness authority: settlement does not consult lease ownership, reset locks, writer exclusion, retention ledgers, staged transactions, or resume records.

Retention is integer ordering for positive epochs: sort the proven epochs and keep the highest two. Sweep removes older epochs, abandoned `.mint-*` directories, and the legacy reset quarantine after epoch one exists. The flat store and its sidecars are never proof targets, lock coordinates, or deletion targets, and the operator cannot address them through `release`. Every sweep failure is audited and skipped so it can be retried on a later boot; cleanup is never a boot dependency. `backend store-reset list` classifies epochs read-only, `release <K>` refuses the current epoch and takes the operator socket guard, and `discard` publishes the next epoch regardless of classification under the same guard. A live coordinator therefore refuses either destructive command with `coordinator_socket_in_use`. The old quarantine reader and bounded SQLite diagnostic child survive only to report legacy incidents while the legacy root exists. Production passes the already opened epoch to the KB daemon in `CORAL_KB_DAEMON_STORE_EPOCH`, and the daemon opens that exact epoch path rather than independently selecting the highest visible epoch.
