...
Reset authority and support inspection deliberately point in opposite directions. Only the backend lifecycle can mint `BackendStoreResetAuthority`; one settlement machine in the selection coordinator holds the reset lock, resumes interrupted work, mints one enumerate-classify-enumerate epoch, and publishes at most one incident. It opens a current-format store only at the reusable owned coordinate `.minted/current`, then enters a counter-free claim loop. Each iteration writes the durable `.parked/.in-flight/parked.v1.json` transaction before parking any present name, classifies and opens `store.db` only at that owned parked path, adopts a compatible occupant, or links the already-open minted inode into the vacant shared name. `EEXIST` starts the next epoch; terminal evidence moves to its UUID coordinate only after its terminal sidecar is durable. Recovery also relocates an interrupted fixed coordinate whose sidecar is absent, malformed, unavailable, or already terminal, preserving the whole directory while freeing the fixed name. One pathname owner mints `{ dev, ino }` identities and mediates every shared-name observation, rename, and atomic link-back; no writable SQLite open receives a shared path. Preserved incident evidence is always copied onto a separate inode because a hard link remains writable through any uncooperative descriptor; legacy linked evidence is re-described as a torn copy before recovery accepts its manifest. The additive-tolerant ledger owns only the in-flight publication promise, preserved slot, historical discard receipts, over-bound preservation status, and validated stored Coral version. Parking records are indexed independently of incident records, so list reports both rows and recursive whole-directory byte counts separately. Release accepts an exact listed parking coordinate, including the fixed in-flight coordinate or a record with an unreadable sidecar, and removes the same whole directory that list describes. SQLite integrity work receives verified copies in a private temp directory, runs in a bounded child process with read-only/query-only pragmas, re-hashes the source evidence, and cannot import the backend reset boundary.
