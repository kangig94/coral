...
A workflow slot is stable semantic lineage; a child job is one runtime execution of that slot. Each newly launched child therefore receives a separately minted canonical UUID job id, while durable child rows carry the slot identity separately. Recovery reconstructs the current mapping by reusing the durable job id for a slot with a current child row, mints an id for a slot without a row, and mints a new id for the next replacement generation when a pending continuation lease requires one. Operator surfaces preserve the same distinction by carrying and displaying slot identity separately, and a proxy launch rejection identifies the operation-identity component it rejected. All newly created and all proxy-routed operation identities are canonical UUIDs.

The rejected alternative was to widen the provider-proxy operation-identity schema to accept slot-shaped values as job ids. During a hot upgrade, a new coordinator and previous-build proxies coexist; the coordinator could then send the widened value to a proxy whose strict UUID schema rejects it before the provider starts. Keeping the proxy wire job identity canonical-UUID-shaped across generations avoids that mixed-version failure instead of making slot identity double as job identity. Recovery recognizes only a verified pre-upgrade queued provider child whose durable launch has `jobId === workflowSlotId`, workflow ownership, and the matching parent. That legacy child retains its identity and is forced onto coordinator-local app-server placement. Unrelated slot-shaped jobs follow normal placement and, if a proxy is selected, are rejected at the strict proxy identity boundary before the provider starts.
