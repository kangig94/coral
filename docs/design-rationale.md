...
Retention is integer ordering. The current epoch and its immediate predecessor are non-garbage; sweep removes `K <= current - 2`, abandoned `.mint-*` directories, the legacy reset quarantine after epoch one exists, and the four flat epoch-zero names after epoch two exists. Every sweep failure is audited and skipped so it can be retried on a later boot; cleanup is never a boot dependency. `backend store-reset list` classifies epochs read-only, `release <K>` refuses only the current epoch, and `discard` publishes the next epoch regardless of classification. The old quarantine reader and bounded SQLite diagnostic child survive only to report legacy incidents while the legacy root exists. The KB daemon independently resolves the highest epoch when its write runtime starts, so no eager database pathname crosses the process boundary.
