...
Reset authority and support inspection deliberately point in opposite directions. Only the backend lifecycle can mint `BackendStoreResetAuthority`; the selection coordinator's first in-place classification of the active pathname is advisory and routes control without authorizing a mutation. Absent, fresh, and compatible stores proceed without the reset lock or a classification copy. A reset or interrupted reset takes the lock, resumes interrupted work, and publishes at most one incident after reclassifying evidence at its owned staged inode. One `SettlementAuthority` refreshes and re-proves adoption, proven writer exclusion, and the reset lock immediately before each guarded act. It opens a current-format store only at the reusable owned coordinate `.minted/current`, then enters a counter-free claim loop. Each iteration writes the durable `.parked/.in-flight/parked.v1.json` transaction before parking any present name, classifies and opens `store.db` only at that owned parked path, adopts a compatible occupant, or links the already-open minted inode into the vacant shared name. `EEXIST` starts the next epoch; terminal evidence moves to its UUID coordinate only after its terminal sidecar is durable. Terminalization probes the canonical evidence names and renames the owned directory whole, so an entry-count overflow cannot refuse boot. Recovery also relocates an interrupted fixed coordinate whose sidecar is absent, malformed, unavailable, or already terminal, preserving the whole directory while freeing the fixed name. One pathname owner mints `{ dev, ino }` identities and mediates every shared-name observation, rename, and atomic link-back; no writable SQLite open receives a shared path. Preserved incident evidence is always copied onto a separate inode because a hard link remains writable through any uncooperative descriptor; legacy linked evidence is re-described as a torn copy before recovery accepts its manifest. The additive-tolerant ledger owns only the in-flight publication promise, preserved slot, and validated stored Coral version. A new incident is committed durably before the previous preserved directory is removed; the ledger names the new holder only after that removal is durable. Parking records are indexed independently of incident records, so list reports both rows and recursive whole-directory byte counts separately. Release removes a committed incident, a contained same-ID terminal or unverifiable parking directory, or both; an unverifiable sidecar makes parking byte accounting unknown but does not block deletion. A verified in-flight transaction, unsafe parking path, or undeterminable containment remains untouched. SQLite integrity work receives verified copies in a private temp directory, runs in a bounded child process with read-only/query-only pragmas, re-hashes the source evidence, and cannot import the backend reset boundary.
