...
This change is a destructive store epoch, not a migration. Durable journal and projection shapes expose no local migration versions and select no upcaster, old-format decoder, or compatibility path. One active `StoreFormatFingerprint` covers the exact executable SQL manifest, registered event type/stream/body/materializer mapping, append-validation semantics, complete job/session/cursor and selected KB authority rows, the component codecs used by other persisted rows, and provider-owned profile, binding, and continuity codecs; any change to that manifest moves the fingerprint — the current transition moves it by adding `projection_jobs.work_dir` with its authority constraint, validating provider/workflow `request.cwd` in the launch body, and carrying the new column in the projection-row codec. Reads use those same contracts before interpreting stored state; indexed hot paths run only after a full current-format authority audit. Before classifying store bytes, cold start consults `active-store-selection.v1.json`. A selection naming a valid newer local build hands off to that build; over a `newer-incompatible` store, an absent, malformed, or invalidated selection automatically publishes a V3 `newer-incompatible-invalid-target` incident and initializes fresh state. Neither outcome requires operator action. The explicit offline discard command applies the same decision by replaying the operator's invocation against a valid newer local target through the handoff runner, so that target performs the discard while the current build leaves the store alone; otherwise it creates the same incident as startup. Ordinary boot also handles `older-incompatible` and `corrupt-or-unsupported` inside `store.db.reset.lock` by automatically publishing an incident and initializing fresh state. `store_newer_incompatible` is `refuseIncompatibleBackendStore`'s own classification refusal, not a selection-protocol outcome: on both the startup and the operator-discard passes, `authorizeClassifiedStore` always resolves a `newer-incompatible` classification into a V3 incident (or, if that publish fails, `store_reset_quarantine_failed`) before that refusal could run. It survives only in `openOrResetBackendStoreDb`'s standalone classification, a call shape exercised in current code only by tests invoking it directly with an explicit path, outside the selection protocol.
...
The quarantine is retained as a current-build **incident**, not as a recovery format. That distinction keeps the destructive epoch model honest: a report can explain which fingerprint/build reset occurred and whether retained bytes still match its manifest, but nothing can reopen those bytes as Coral state. Automatic-reset manifests use schema V3 and require the closed `resetPolicyCause` set `older-incompatible | corrupt-or-unsupported | newer-incompatible-invalid-target`; startup authors all three values, with the third produced by the active-store selection protocol's invalid-target classification. V2 remains readable for diagnostics but is never eligible for automatic resume. An interrupted automatic reset resumes only for a self-authored V3 manifest from the same validated authority, canonical store, and flavor with a resettable cause. Committed staged evidence controls whether the commit can finish; an unmatched or undeterminable active name is left for immediate reclassification rather than promoted into a new boot refusal. Refusals preserve the classification in their public code: `store_reset_interrupted_ambiguous`, `store_reset_interrupted_foreign`, `store_reset_interrupted_mismatched`, `store_reset_interrupted_authority_mismatch`, `store_reset_interrupted_malformed`, or `store_reset_interrupted_non_resettable`. Only unexpected quarantine I/O uses `store_reset_quarantine_failed`; every refusal stays operator-visible.

Reset authority and support inspection deliberately point in opposite directions. Only the backend lifecycle can mint `BackendStoreResetAuthority`; one settlement machine in the selection coordinator holds the reset lock, resumes interrupted work, classifies the next epoch, publishes every disposition, and opens the database once after the bounded loop. The reset boundary selects link or copy from the generation-maintenance disposition, stages DB/WAL/SHM/format evidence from active names, atomically parks those names in the sibling private `.parked/` namespace, and only then commits the incident manifest. One pathname owner mints `{ dev, ino }` identities and mediates every shared-name read, rename, and atomic link-back; callers receive no active path or unlink capability. A matching inode is dropped only through its owned parked name. A replacement is linked back without overwrite, or remains parked and is recorded when the claim fails. Linking avoids a size-keyed startup veto after writers drain; copy remains the unbounded fallback and records why linking was unavailable plus whether the copied snapshot was verified coherent or observed torn. One additive-tolerant side ledger owns the parking promise, the preserved slot, kept-parked names, aggregate descendant discards and over-bound preservation status, and the validated stored Coral version that the frozen manifest cannot carry. A pending promise is reconciled before adoption but never acts as the holder. The bounded local list surface reads those facts without opening the incompatible store and returns a marked partial result rather than making an overfull root impossible to drain. `backend store-reset release` is the explicit irreversible capability for deleting one named committed incident and its matching parked namespace; it proves the quarantine and incident directories, takes adoption then the reset lock, reports whether post-removal durability was proven, bypasses IPC, and never appears in automated remediation. SQLite integrity work receives verified copies in a private temp directory, runs in a bounded child process with read-only/query-only pragmas, re-hashes the source evidence, and cannot import the backend reset boundary. This separation makes reporting useful to developers without turning retained evidence into a second mutable store surface.
