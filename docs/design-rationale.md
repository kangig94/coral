...
Reset authority and support inspection deliberately point in opposite directions. Only the backend lifecycle can mint `BackendStoreResetAuthority`, hold the reset lock, select link or copy from the generation-maintenance disposition, durably verify active DB/WAL/SHM/format evidence, commit the incident manifest, remove matching active names, and publish the completed incident. Linking avoids a size-keyed startup veto after writers drain; copy remains the unbounded fallback and records why linking was unavailable plus whether the copied snapshot stayed coherent. One additive-tolerant side ledger owns the preserved slot, aggregate descendant discards and over-bound preservation status, and the stored Coral version that the frozen manifest cannot carry. The local list surface reads those facts without opening the incompatible store. `backend store-reset release` is the explicit irreversible capability for deleting one named committed incident; it takes adoption then the reset lock, bypasses IPC, and never appears in automated remediation. SQLite integrity work receives verified copies in a private temp directory, runs in a bounded child process with read-only/query-only pragmas, re-hashes the source evidence, and cannot import the backend reset boundary. This separation makes reporting useful to developers without turning retained evidence into a second mutable store surface.
