...
The remainder is diagnostic: it names whatever the ledger left, so a closed subject enum would need extending for every obligation the ledger ever gains. The ledger's own `label` is the identity it already carries for every obligation, so the record is keyed by it. Its versioned directory follows the `HANDOFF_CAPSULE_FILENAME` precedent (`src/provider-proxy/handoff-capsule-discovery.ts`): a generation admitted by address, invisible to any reader whose selector predates it, so the mixed-window answer for an older CLI is "nothing". This is [`design-philosophy`](../.claude/rules/design-philosophy.md) §10's rule that a shape which cannot stay additive is a new generation at a new address.
