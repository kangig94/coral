...
| `store/active-store-selection.ts`                       | Active-store selection/transition coordination-record schemas, encode/decode, and trust-boundary bounded read | `active-store-selection.v1.json` / `active-store-transition.v1.json` under the coordination root | `infra/bundle-manifest.ts` and `infra/handoff-target.ts` types | Whether to supersede, refuse, or hand off on a given read result |
...
| `cli/handoff-notice.ts`                                 | The one-time stderr delegation notice printed after a CLI invocation is handed off | `process.stderr` only                                | `HandoffSuccess` from `coordinator/handoff-runner.ts`        | Command result emission (`emit.ts`)            |
...
| `infra/process-containment.ts`                          | Recorded-containment identity verification and the SIGTERM→SIGKILL reap of a proxy process group plus provider roots by pid + start time | Process signals to verified targets only | `infra/monotonic-clock.ts`, `infra/node-process.ts`          | Which processes to record or when to reap (caller's bound) |
