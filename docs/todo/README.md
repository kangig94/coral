...
| [`shutdown-remainder-has-no-reader.md`](./shutdown-remainder-has-no-reader.md)                                               | **Track B of #357.** The no-daemon `backend status` arm now reports the newest recent shutdown remainder. The remaining question is how a live coordinator projects a held boundary's reason and exit through health, without inventing a `transfer-pending` lifecycle state.                                                                                                                                                                             |
| [`shutdown-abandonment-offer-is-dead-in-the-coordinator.md`](./shutdown-abandonment-offer-is-dead-in-the-coordinator.md) | **Unreachable by construction, kept for the mixed window.** `foldRetainedAuthority` never offers an abandonment, so the coordinator's abandon lookup and every `shutdownObligationAbandoned` branch are dead; the CLI command, schemas and IPC route stay for a newer CLI against an older daemon. Delete the coordinator side, keep the wire. |
| [`status-solicits-a-provider-proxy-set-remedy.md`](./status-solicits-a-provider-proxy-set-remedy.md) | **The offer that is still reachable.** `formatProviderProxySetOperatorExit` prints a contain or abandon command into the primary status surface, to a reader who cannot judge it and will run it. Unlike the coordinator's dead shutdown offer, deleting these lines decides nothing — the set still has to be disposed of, so the entry asks which of principle 11's two remaining exits each arm gets, and what deadline decides for itself. |
...
| [`shutdown-remainder-unreadable-record-has-no-exit.md`](./shutdown-remainder-unreadable-record-has-no-exit.md) | **A hold whose only exit is 31 siblings.** A record whose read keeps being refused joins the `unreadable` bucket and is evicted only by being outranked past `MAX_SHUTDOWN_REMAINDER_RECORDS`, which on a healthy install never happens. The review's framing that it warns forever was corrected in place — the prune runs once per boot, so what actually repeats is the filename crossing `skippedUnreadableRecordNames` on every `backend status`. Principle 11's visibility half is satisfied only for the directory-scoped statuses — a `recorded_process_absent` read from a legacy discovery record carrying no `instanceId` narrows to nothing and surfaces no unreadable record at all — and the half that names what ends it is not satisfied anywhere. |
| [`shutdown-remainder-predecessor-quarantine-was-unreleased.md`](./shutdown-remainder-predecessor-quarantine-was-unreleased.md) | **Closed provenance decision.** The nested `quarantine/<subject>/<slot>/evidence` writer existed for one unreleased branch commit and never entered `clients/bridge`; no reconciler is warranted. Current readers name its top-level entry as unrecognized and current pruning cannot call that directory complete. |
...
| [`shutdown-remainder-tests-pin-the-whole-status.md`](./shutdown-remainder-tests-pin-the-whole-status.md) | **The guard rots on the edit principle 10 sanctions.** Several `toEqual` assertions pin the entire decoded record where the test's own name claims a narrow property, so adding an optional field — the only edit an additive-only durable record permits — breaks tests that check nothing about it, and a failure does not say what regressed. The neighbouring `toMatchObject` sites already do it right; the companion twenty in the transport suite belong to that subsystem. |
