...
| [`provider-proxy-heartbeat-hold-status.md`](./provider-proxy-heartbeat-hold-status.md)       | **Implemented.** Set and acquisition holds have a durable owner, predecessor records are stale evidence until exact re-observation, and atomic replacement binds new records to retired keys. A store failure retains the hold through its named retry exit.                                                            |
| [`durable-cli-signal-authority.md`](./durable-cli-signal-authority.md)                       | **Implemented.** Durable CLI containment carries identity-safe signal authority, publishes durable refusal status, retries while ownership is held, and requires explicit operator abandonment before releasing an unproven-absent obligation.                                                                          |
| [`containment-observation-deadline.md`](./containment-observation-deadline.md)               | **Closed.** Budgeted containment uses asynchronous identity-bound observation, checks its deadline around awaited probes, and returns `unobservable` rather than deriving absence when observation cannot finish within the bound.                                                                                       |
...
| [`ralph-hook-misquotes-its-own-skill.md`](./ralph-hook-misquotes-its-own-skill.md) | **Startable now; the defect is reproduced.** The hook's injected plan-mode instruction deletes the state file that the Ralph skill requires for loop continuation, and the missing-state guard then exits silently. Correct the injected instruction, cover plan-mode continuation, and make lost session state diagnosable. |
