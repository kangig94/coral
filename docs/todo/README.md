...
| [`provider-proxy-heartbeat-hold-status.md`](./provider-proxy-heartbeat-hold-status.md)     | A heartbeat hold ends on role liveness evidence or the coordinator's bounded escalation, but its current state lives only in the coordinator lifecycle and logs. Choose a durable owner and stale-record rule, then project the exact set/role/method through the existing `coral-cli backend status` command. |
