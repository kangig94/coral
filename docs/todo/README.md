...
|                                                                                |                                                                                                                                                                                                              |
| ------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| [`unit-suite-concurrency-and-real-time-tests.md`](./unit-suite-concurrency-and-real-time-tests.md) | **Half closed: file-backed store opens are rejected in the concurrent tier.** Test database opening had three helper homes, including two duplicates for formatted stores. The formatted variants now share one module, every database door validates the opened handle's location against the stamped tier, and an invariant scans all TypeScript under `tests/` and `tools/testing/` for named imports and direct namespace-property accesses of the two raw openers outside the checked doors. `tests/integration/` and `tests/e2e/` are deliberately excluded because direct opening is often the behavior under test; integration runs with one worker. The real-time half is measured and small. |
