...
| ----- | ----------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1     | `session-start-context-truncated`                           | Measured, not inferred: the SessionStart packet is 11,320 bytes against a threshold that shows the first 2 KB, so every session reads the general advice and loses the operative rules. Option 1 is a reordering inside one file. Everything else in this directory assumes an agent that read them. |
