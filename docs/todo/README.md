...
| [`process-port-answers-with-two-values.md`](./process-port-answers-with-two-values.md)                   | **The owner of the primitives is the one over-claiming, and the gate for it is now in place.** Sixteen review rounds of "unknown read as proof" came not from bypassing `ProcessPort` but from four of its members answering two ways — `readProcessIncarnation` returning `null` for absent *or* unreadable *or* unprobed, `kill` collapsing ESRCH and EPERM into `false`, `spawn` deferring its failure to a later event, `gracefulKill` returning nothing while its pid twin returns a disposition — so roughly fifty-five callers reconstruct the third answer by hand. `process-observation-composition` now refuses new ones and carries the unconverted boundaries as a self-pruning ledger; converting them is compiler-driven and deliberately deferred past the branch, because it touches the files with the heaviest churn on it. |
