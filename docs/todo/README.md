...
| [`project-source-undecidable.md`](./project-source-undecidable.md)               | **Durable half closed 2026-08-18; one call's worth remains.** `resolveProjectSource` returns one `string` for "no git remote" and "the probe timed out", and `projectData` derives a KB memo directory from it — so a call made while a mount is stalled files a memo where later reads do not look. A timed-out probe is no longer cached, so a recovered mount self-heals; the guess inside that window is still indistinguishable from an answer. Closing it means a disposition in a port return type every consumer assumes always has a value.                                                                                                                                                           |
