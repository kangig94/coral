...
| [`terminal-body-grammar-outside-the-generation.md`](./terminal-body-grammar-outside-the-generation.md) | **The address covers part of the grammar and the documentation said it covered all of it.** The routing journal's fingerprint hashes the SQL, its bounds, and the disposition/routing-basis vocabularies — but the strict Zod bodies hold enumerations no hashed input mentions (`resolutionReasonSchema` is the plain case; two of eleven `z.enum` sites are pinned at all). Widen one and both builds share the address, so the older reader answers `unreadable` — this journal is damaged — where the truth is that another build wrote it. Bounded today because no released build writes the artifact. Closing it is either every enum plus a source-scan invariant, or one declarative grammar both the schemas and the fingerprint derive from; **not** a hand-maintained version number, which is what this mechanism just had removed. |
...
| [`provider-proxy-heartbeat-hold-status.md`](./provider-proxy-heartbeat-hold-status.md)     | Any answer ends the exact silence hold, while only an accepted echo or resynchronization also ends answered-but-unusable evidence; bounded exits remain coordinator-local and visible only in logs. Choose a durable owner and stale-record rule, then project the exact set/role/method through the existing `coral-cli backend status` command. |
...
| [`operator-shutdown-cannot-reap-a-live-set.md`](./operator-shutdown-cannot-reap-a-live-set.md) | **Named as an exit and is not one.** Both routes into shutdown hard-code `reason = 'replaced'`, which maps to handoff, and handoff preserves live proxy sets for a successor to redeem — so an operator's `backend shutdown` hands a wedged set to the next coordinator rather than ending it. Hard mode reaps but no command reaches it. Preserving work across an upgrade is right; one reason serving both situations is the defect. |
| [`control-channel-eof-is-not-process-absence.md`](./control-channel-eof-is-not-process-absence.md) | A socket EOF or malformed frame is positive evidence about one connection, not proof that the coordinator process or its work is absent. `reattachControl` already models the same holder returning, but bootstrap control has no re-provable credential after its one-shot nonce is spent; choose that credential before moving `control-channel-fault` off the terminal latch. |
...
| [`handoff-wall-clock-graces.md`](./handoff-wall-clock-graces.md)                 | **The handoff deadline and both signal graces measure elapsed time with `Date.now()`.** A backward adjustment stalls escalation; a forward one observes a post-grace outcome before the grace elapsed. The `runtime.time.now()` calls that supply the current handoff state machine's deadline, signal timestamps, and remaining-budget decisions must move to one monotonic clock as a separately reviewable change.                                                                                                                                                                                                                                                                |
...
then migrates every `runtime.time.now()` call that supplies the current handoff state machine's elapsed-time
decisions together.
...
| [`socket-address-ownership.md`](./socket-address-ownership.md)         | **Current-build installation identity closed; shipped-selector compatibility blocked.** Relocated current paths derive from the state root, so caller uid and `TMPDIR` cannot split two current builds; a caller that cannot own the shared installation directory refuses. v0.10.9 accepts an arbitrary inherited `TMPDIR`, so no finite compatibility-listener set can guarantee collision with every later shipped invocation. The three provider role binders still inherit an assertion made in another process, and the owner/mode assertion still cannot observe macOS ACL grants. |
...
|                                                                                |                                                                                                                                                                                                              |
| ------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| [`unit-suite-concurrency-and-real-time-tests.md`](./unit-suite-concurrency-and-real-time-tests.md) | **Every gate is green and the suite starves the machine it runs on.** An unbounded fork pool reached ~108 node processes with 15 in uninterruptible sleep on the ext4 journal. Unanswered 5 s heartbeats are now retained and retried, but a stall beyond the enforcer's adoption deadline can still terminate unrelated jobs; twelve delegated jobs died under the earlier immediate-fault policy in one day. The suite is now capped at 8 local workers, which halves the stall depth for 1.9x the wall time and does not change how often a stall happens, so it is a cap and not a fix. **The tmpfs result is device-conditional:** it changed nothing while fsync cost about 1 ms, but after the device degraded to roughly 300 ms per fsync, routing the suite's temp root to tmpfs removed the dominant temp-file cost. The remaining lever for durable SQLite work is the journal mode, not the durability pragma. |
