...
| [`control-channel-eof-is-not-process-absence.md`](./control-channel-eof-is-not-process-absence.md) | **Channel policy implemented; exact-set operator exit still missing.** EOF and malformed frames are incidents, not terminal faults. The lifecycle holds claims under one adoption-window deadline while authenticated redemption races independent absence; refusal and expiry wait for absence without reaping. Ordinary `backend shutdown` is explicitly not an exit because it hands the set forward. |
