...
| [`shared-tmp-ownership.md`](./shared-tmp-ownership.md)                 | **Mostly closed.** The Coral-owned paths under a root every user on the host can write, which the socket fix left standing. The three files in a job directory — including the `env.json` that carries an unfiltered child environment — are now `0600`; the Bash hook's inline-text spill no longer names itself after its own content, and `wx` makes it refuse a pre-existing entry rather than write through it; the KB curate corpus asks for a mode. What is left is the mode on the literal `/tmp/coral-jobs` itself, which cannot be set before the per-user naming question `socket-address-ownership.md` holds is answered, and the harness-owned `/tmp/claude-<uid>`. |
