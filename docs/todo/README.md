...
| [`provider-proxy-heartbeat-hold-status.md`](./provider-proxy-heartbeat-hold-status.md)     | Each role and method has one exclusive clear, silence, or answered-unusable evidence window, and the live coordinator projects it through `coral-cli backend status`. Choose a durable owner and stale-record rule so that status survives coordinator death without presenting stale holds as current. |
...
| [`control-channel-eof-is-not-process-absence.md`](./control-channel-eof-is-not-process-absence.md) | **Channel policy implemented; exact-set operator exit still missing.** EOF and malformed frames are incidents, not terminal faults. The lifecycle holds claims under one adoption-window deadline while authenticated redemption races independent absence; refusal and expiry send no destructive control RPC, but the independent proof reaps the orphaned proxy group after both enforcers are proven absent. Ordinary `backend shutdown` is explicitly not an exit because it hands the set forward. |
