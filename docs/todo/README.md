...
| [`control-channel-eof-is-not-process-absence.md`](./control-channel-eof-is-not-process-absence.md) | **Implemented, including `backend provider-proxy-set contain`.** EOF and malformed frames are incidents, not terminal faults. Authenticated redemption races independent absence; after both enforcers are absent, the proof reaps the recorded proxy group and recorded provider roots. Ordinary shutdown still hands the set forward. |
