...
| [`launch-slot-released-into-the-wrong-pool.md`](./launch-slot-released-into-the-wrong-pool.md) | **Resolved.** Release now requires the exact generation-bound permit, reports its disposition, and exposes reportable holders. The field measurements did not prove that wrong-pool release caused the incidents. |
