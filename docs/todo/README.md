...
| [`provider-proxy-heartbeat-hold-status.md`](./provider-proxy-heartbeat-hold-status.md)     | A heartbeat hold now ends only on an accepted echo, but its current state lives only in the coordinator lifecycle and logs. Choose a durable owner and stale-record rule, then project the exact set/role/method through the existing `coral-cli backend status` command. |
