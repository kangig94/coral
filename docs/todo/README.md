...
**One and two share the same measured load, but the destructive classification half is closed.**
`store-lock-misread-as-corruption` remains first for its two open handoff defects: a process still holding the
socket with `SIGKILL` pending is treated as though the signal decided its exit, and a signal cooldown is
announced as manual repair even though time ends it. `unit-suite-concurrency-and-real-time-tests` is what makes
the incident's load ordinary: a suite run saturates the one filesystem that the repo, `~/.coral` and `/tmp`
all share, leaving the coordinator blocked mid-fsync. Fixing the store classification removed the destructive
advice; it did not fix either handoff disposition or the load that exposes them.

| Order | Entry                                                       | Why here                                                                                                                                                  |
| ----- | ----------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1     | [`store-lock-misread-as-corruption`](./store-lock-misread-as-corruption.md) | The destructive store-classification half closed 2026-08-26. What remains is the handoff half observed in the same incident: `SIGKILL` pending on a process in uninterruptible sleep is treated as a decisive exit attempt, and the bounded signal cooldown is announced as manual repair instead of naming time as its exit. |
| 2     | [`unit-suite-concurrency-and-real-time-tests`](./unit-suite-concurrency-and-real-time-tests.md) | Measured 2026-08-25: this manufactures the load behind entry 1. One `npm test` holds the device at 100% utilization for 73% of the run with 368 requests queued, so a coordinator sharing that filesystem loses its heartbeat and can block mid-fsync while holding the store lock. The classification half no longer calls that lock corruption, but the load still exposes entry 1's handoff defects and makes every gate result uncertain — adding one 2 Hz sampling loop failed four unrelated tests that pass alone. The lever is `journal_mode=WAL`, not `synchronous=FULL`: dropping the WAL is 473x where relaxing durability alone is 2.6x. |
...
| [`control-deadline-cannot-tell-silence-from-starvation.md`](./control-deadline-cannot-tell-silence-from-starvation.md) | **Field-observed: eight claim-bearing reaps in one log.** `control-lost` is not a peer's answer — `echoChallenge` returns it from a comparison of the local monotonic clock against `controlLossAt()`, so a process whose timers wake 10,488 ms late cannot tell "the peer stopped answering" from "I was not scheduled to hear it", and the set is stopped and reaped with `liveClaims=1`. The evidence needed to discount the verdict is already measured and never consulted: the same log reports each timer's own lateness. What starved the coordinator is deliberately **not** claimed — two candidates were in flight and neither was isolated. |
...
| [`store-lock-misread-as-corruption.md`](./store-lock-misread-as-corruption.md)                           | **Store classification closed; two handoff dispositions remain.** Busy/locked, corrupt/unsupported, and unclassified store-open failures now have separate outcomes, so a lock no longer reaches discard advice. The remaining probe-answer defect sits beside [`handoff-escalation-never-reaches-the-operator.md`](./handoff-escalation-never-reaches-the-operator.md): that entry is about the message not arriving; this one says a socket still bound with `SIGKILL` pending does not prove the incumbent exited or that the socket is anomalous. The same incident also leaves the signal cooldown mislabeled as manual repair even though time ends it. |
