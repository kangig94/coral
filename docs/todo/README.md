...
| [`store-reset-bound-promoted-to-a-boot-refusal.md`](./store-reset-bound-promoted-to-a-boot-refusal.md) | **Closed historical design record, not a specification.** The shipped design publishes complete stores under positive write-once `epoch-<N>` directories with one rename, selects the highest proven epoch, and retains the highest two. It never opens, locks, or deletes the previous generation's flat store, and it has no copy/quarantine/size-bound reset path. Thirty-five revisions and forty-three tier-1 review rounds preserve how the design reached that smaller shape; the current-state lead is authoritative over the chronological record below it. |
