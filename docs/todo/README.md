...
| [`scoped-ignore-glob-anchoring.md`](./scoped-ignore-glob-anchoring.md) | **Closed 2026-08-31.** A slashless gitignore pattern matches at any depth below its owning file. The unreleased working-tree temp pattern was deleted, while the surviving `.claude/coral` rule moved to `.git/info/exclude` as a literal-escaped, project-anchored path. The close-out also records the lower precedence of the new location. |
