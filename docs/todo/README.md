...
| [`shutdown-remainder-unreadable-record-has-no-exit.md`](./shutdown-remainder-unreadable-record-has-no-exit.md) | **A hold whose only exit is 31 siblings.** A record whose read keeps being refused joins the `unreadable` bucket and is evicted only by being outranked past `MAX_SHUTDOWN_REMAINDER_RECORDS`, which on a healthy install never happens. The review's framing that it warns forever was corrected in place — the prune runs once per boot, so what actually repeats is the filename crossing `skippedUnreadableRecordNames` on every `backend status`. Principle 11's visibility half is satisfied; the half that names what ends it is not. |
