...
| [`explicit-drain-waits-behind-inflight-gate.md`](./explicit-drain-waits-behind-inflight-gate.md) | **Owner decision, and the first attempt was wrong.** An explicit drain stays before the ledger while a unary request is in flight. Carries the measurement that the sequence does *not* inherit that wait — the listening socket starts closing before the in-flight obligation runs. |
