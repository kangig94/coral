...
| [`representation-release-retries-forever.md`](./representation-release-retries-forever.md) | **Narrowed to its cause.** The *logged* loop that produced 72,641 identical WARN lines over 28 hours is gone, and so is the unbounded hold on a representation slot: a 60-second deadline armed against the slot expires even when no delivery ever settles, and expiry releases the slot as `released-undischarged`, naming whichever artifact is still outstanding — the provider-operation record the reconciler's own poll re-attempts, or the handoff capsule the next boot's discovery reclaims. The re-attempt itself is unbounded by design, and the record it names is now told what it witnessed: each failed attempt advances its retry accounting and writes the real terminalization cause, reported once per change of cause rather than once per attempt. What remains is why it retried at all — every non-journal throw inside the synchronous `store.commit` closure is classified `retry-safe-unknown`, so a deterministic failure is retried as a transient one. Separating them needs the store to report which stage failed; from outside `withImmediate` a closure throw and a `COMMIT` throw are indistinguishable, and decisive is not the same as deterministic. |
...
| [`shutdown-remainder-unreadable-record-has-no-exit.md`](./shutdown-remainder-unreadable-record-has-no-exit.md) | **Resolved by having one address.** The directory of per-instance records, its retention ranking, its cap and its pruner are gone; the remainder is one file. A record this build cannot read is reported with its path and the classification observed (`unreadable`, `corrupt`, `unsupported`), and its exit is the next shutdown of any kind renaming over it — the writer now publishes on every shutdown, because an exit reachable only from the pathological case is not an exit. Both premises the old bounded store rested on — that a readable record's age can be unknown, and that the directory could grow to disk capacity — were refuted by measurement. |
...
| [`representation-release-notice-as-a-durable-phase.md`](./representation-release-notice-as-a-durable-phase.md) | **A direction, with the two obstacles that stopped it named.** The release notice is an in-memory latch beside a durable record that now carries the same work's attempt accounting; making it a durable phase of the record would delete the latch, both delivery machines and the unawaited kick. Two things have to be shown first: the latch's second job is an abort **fence**, which a revision compare-and-swap refuses but does not abort, and every record schema is `.strict()`, so a new phase is a generation question under principle 10. |
