...
| [`foreign-capsule-retirement.md`](./foreign-capsule-retirement.md)                   | **Harm removed, residue kept — and it still holds a rollback-unsafe write.** A handoff capsule this build may not act on is represented so its address cannot be aliased, and dialing one is fatal rather than merely useless — but nothing retires the file, so it is rediscovered and re-warned every boot, and `#rewriteOpaqueCapsule` still writes current-generation bytes at a legacy filename. Absence is provable from the recorded pids without redeeming or signalling anything; what it needs is a process port and an observe-then-retire turn. |
