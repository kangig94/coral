...
|                                                                                                                              |                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| ---------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [`unauthorized-status-remedy-cannot-act.md`](./unauthorized-status-remedy-cannot-act.md) | **The token-mismatch remedy prints a command the coordinator rejects.** `unauthorized` tells the reader to run `backend shutdown`, which presents the same rejected token and fails as `capability_rejected`, then to retry a mutating command, which cannot authenticate or replace that coordinator either. Found while adding `backend start`, which deliberately adds no start line there. |
...
| [`provider-session-residue-has-no-owner.md`](./provider-session-residue-has-no-owner.md) | **Retention discards the one file it can locate; providers write more.** Codex forks mid-job (899 left in one five-week window) and claude writes a `tool-results/` directory beside each session (124 of 124 completed sessions, 27 MB). Archive-then-discard itself is clean. Settled: both are discard-only, re-derived at every retention attempt from the durable root id and discarded before completion, so no continuation shape changes. The accumulated backlog is a separate sweep afterwards. |
