...
| [`provider-proxy-heartbeat-hold-status.md`](./provider-proxy-heartbeat-hold-status.md)     | Each role and method now has one exclusive clear, silence, or answered-unusable evidence window; bounded exits remain coordinator-local and visible only in logs. Choose a durable owner and stale-record rule, then project the exact set/role/method through the existing `coral-cli backend status` command. |
