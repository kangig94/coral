...
| [`source-mode-build-identity-sentinel.md`](./source-mode-build-identity-sentinel.md) | **The cause behind that deletion.** Every source-mode run claims one fixed `buildSetId`, so the gate that decides whether one build may dial another's processes answers "same build" for two unrelated source trees. One consequence was found and removed; the gate still returns a confident wrong answer. Absence is provable from the recorded pids without redeeming or signalling anything; what it needs is a process port and an observe-then-retire turn. |
