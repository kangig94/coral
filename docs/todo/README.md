...
| [`no-store-migration-path.md`](./no-store-migration-path.md)                                           | **The reason every other entry in this section exists.** `classifyStoreFormat` has one non-destructive outcome for a store carrying version metadata, and `legacy-adoptable` is a rewrite-era adoption rather than a migration — so any release whose `schema.sql` moves the fingerprint destroys every store on upgrade, silently and automatically. Measured: 9 commits to that file against 33 release tags. The fingerprint cannot tell an additive change from a destructive one, which may be most of the fix. Sibling of the row below, and they do not close together: that one keeps more than one store, this one carries one store forward. |
...
| [`store-reset-bound-promoted-to-a-boot-refusal.md`](./store-reset-bound-promoted-to-a-boot-refusal.md) | **Design settled; this document is the specification.** Nothing prunes the store-reset quarantine root either, and the reason the gap surfaced is that a 1 GiB limit on *hashing a report* refused a boot outright — recovering it took a plugin rollback by hand. Preservation moves bytes with `link(2)` when the generation maintenance lease proves writers are drained and copies when it cannot, so size never blocks startup; the bound is one preserved slot enforced by refusing to add, never by evicting, because a later incident preserves a shorter suffix than the one it would displace. Three review rounds each found the same mistake in new clothing: a bound correct about what it measured, promoted into a veto on booting. |
