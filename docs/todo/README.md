...
| [`job-scratch-ownership.md`](./job-scratch-ownership.md)               | The other Coral-owned path under a root every user on the host can write. `/tmp/coral-jobs` is a literal, created with no mode, and the `env.json`, `stdout`, and `stderr` beneath it are written with none either — so a UUID job id stops being unguessable the moment someone else owns the parent and lists it. The namespace half shares `socket-address-ownership.md`'s undecided question; the file modes do not and are worth doing alone. |
