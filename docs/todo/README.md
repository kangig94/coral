...
| [`operator-shutdown-cannot-reap-a-live-set.md`](./operator-shutdown-cannot-reap-a-live-set.md) | **Implemented with a separate exact-set exit.** Ordinary `backend shutdown` remains handoff and preserves live proxy sets; `coral-cli backend provider-proxy-set contain <pps1-token>` is the explicit held-set operation with proof, refusal, and abandonment outcomes. |
| [`control-channel-eof-is-not-process-absence.md`](./control-channel-eof-is-not-process-absence.md) | **Implemented, including `backend provider-proxy-set contain`.** EOF and malformed frames are incidents, not terminal faults. Authenticated redemption races independent absence; after both enforcers are absent, the read-only prover supplies exact targets and the lifecycle reaps the recorded proxy group and provider roots. Ordinary shutdown still hands the set forward. |
