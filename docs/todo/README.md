...
| [`shared-tmp-ownership.md`](./shared-tmp-ownership.md)                 | The Coral-owned paths under a root every user on the host can write, which the socket fix left standing. `/tmp/coral-jobs` is a literal created with no mode, and the `env.json` beneath it holds an unfiltered child environment; the Bash hook's inline-text spill names its file after the content's own hash and writes through whatever that name resolves to; the KB curate corpus carries no mode. The per-user naming — and the directory mode that cannot be set without it — shares `socket-address-ownership.md`'s undecided question. The file modes and the hash-named file do not, and are worth doing alone. |
