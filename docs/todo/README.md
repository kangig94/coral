...
| [`macos-socket-directory-acl.md`](./macos-socket-directory-acl.md)                                       | **Owner plus mode is not effective access on macOS.** The relocated socket-directory check proves the expected uid, directory type, mode `0700`, and its parent mode premise through Node `fs`; an allow ACL can still grant another principal access while every observed field passes. Node exposes no ACL entries, so closing this needs a macOS ACL observation port, a refusal mapping for unreadable and permissive ACLs, and a real ACL fixture. |
...
| [`socket-address-ownership.md`](./socket-address-ownership.md)         | What the socket-identity fix left open by moving the overflow fallback under a root shared with every user on the host. The coordinator's binder asserts the directory it binds in; the three provider role binders inherit an assertion made in another process, which has expired by the time they bind — and moving the check to them needs a startup-diagnostic channel they do not have. And once a socket relocates, the uid participates in its address, so one state root under two uids is two locks — `design-rationale.md` §8.2 says one coordinator per installation and never says whether the uid, real or effective, is part of one. |
| [`shared-tmp-ownership.md`](./shared-tmp-ownership.md)                 | The Coral-owned paths under a root every user on the host can write, which the socket fix left standing. `/tmp/coral-jobs` is a literal created with no mode, and the `env.json` beneath it holds an unfiltered child environment; the Bash hook's inline-text spill names its file after the content's own hash and writes through whatever that name resolves to; the KB curate corpus carries no mode. The per-user naming shares `socket-address-ownership.md`'s undecided question. The modes and the hash-named file do not, and are worth doing alone. |
