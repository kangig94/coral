...
several sessions at once: 46 scheduler stalls of 5 s or more, worst 68 s, and every one of the 25
`stop-and-reap` decisions in the logs followed a stall within seconds, each killing healthy delegated jobs
by policy. The shipped 5-second verdict is already corrected in source — `wedged-coordinator-self-drain`
records that, and the logs are 0.10.9 — but the window current source actually tolerates is
`37,000 - 14,000` = 23 s, still under half the requirement. The requirement is settled; **the mechanism is
an open question** with four shapes and no choice made, which is why this leads rather than being a patch.
It ranks above the compatibility policy because that policy decides what two builds may say to each other,
while this decides whether either finishes its work on a loaded machine.
...
|                                                                                            |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| ------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [`overload-tolerance-floor.md`](./overload-tolerance-floor.md) | **Top priority. A machine under load must survive 60 seconds of coordinator starvation without killing work.** 46 measured stalls of 5 s or more, worst 68 s, and every `stop-and-reap` in the logs followed one — 25 of them, each ending healthy jobs the operator then reads as `provider_lost`. The 5-second verdict those logs show is 0.10.9 and is already corrected in source; what current source tolerates is 23 s (`orphanTimeoutMs - teardownReserveMs`), still under half. Requirement settled, mechanism open: move the default, derive the window from observed lateness, change what the off-process enforcer accepts as evidence, or keep "did not answer" out of "is gone". |
