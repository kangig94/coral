...
| [`socket-address-ownership.md`](./socket-address-ownership.md)         | What the socket-identity fix left open by moving the overflow fallback under a root shared with every user on the host. The coordinator's binder asserts the directory it binds in; the three provider role binders inherit an assertion made in another process, which has expired by the time they bind. And the effective uid now participates in the address, so one state root under two uids is two locks — `design-rationale.md` §8.2 says one coordinator per installation and never says what an installation is. |
