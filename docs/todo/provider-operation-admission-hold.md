...
| `cli/backend-status.ts`                                         | a public `held` result naming every `key@revision`, printing the clear command and identifying abandon as an override — never "Backend ok"                                          |
