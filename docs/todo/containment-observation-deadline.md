...
- `probeCoordinator`'s disposition, closed 2026-08-18. Its _read_ half no longer derives an incarnation at
  ...
  none, and `src/coordinator/lifecycle.ts:1016` supplies none, so a darwin boot pays two `execFileSync` calls
...
- **Check the deadline inside the observation loop.** `observeRecordedSet`
  (`src/infra/process-containment.ts:225-234`) already receives an environment carrying `environment.clock`,
