...
| `provider-proxy/`     | Control protocol, bootstrap capsules, each role's own peer-spawning, and the read-only `operation.status.v1` wire handler over its operation ledger                 | Coordinator observation/pass policy, wiring, the store, or the coordinator socket |
| `coordinator/`        | Live state, startup order, cross-domain assembly, carrier observation/pass policy, and cross-build handoff decision/execution                                       | Domain vocabulary or proxy wire-handler ownership                                 |
