...
| `provider-proxy/`     | Control protocol, bootstrap capsules, each role's own peer-spawning, its process-local provider-host authority, and the read-only `operation.status.v1` wire handler | Coordinator observation/pass policy, wiring, the store, or the coordinator socket |
| `coordinator/`        | Live state, startup order, cross-domain assembly, coordinator-local hosts, provider-host federation/routing, carrier observation/pass policy, and cross-build handoff | Domain vocabulary or proxy wire-handler ownership                                 |
