...
The coordinator layer owns process lifecycle, the three-era boot sequence (Kernel → Recovery → Runtime Health Components), projection-consumer wiring, provider-host coordination, job-backed KB source import/reindex proxying, and the corpus notify seam. It also owns the local-evidence half of carrier observation that the classifier deliberately does not: `composition/carrier-observation.ts` gathers the local evidence (admission state, the OS process table, the durable-CLI identity row) and builds the wait stream's observation port. There is no other half — a client-side network observer that would have queried a foreign proxy's operation status once existed alongside it, but it had zero importers, because both of its intended callers were never wired, and it is gone. `tests/invariants/no-carrier-observation-in-action-paths.test.ts` proves no recovery, cleanup, reconciliation, or terminal-materialization path can obtain a verdict at any import depth from the classifier that remains. Recovery's own cross-process check is a different, narrower thing: `infra/process-containment.ts`'s `confirmAbsence` reconfirms a recorded proxy/provider-root containment stays gone before the reap path treats it as settled, never a carrier verdict. The abandoned split's server side survives on purpose — `provider-proxy/proxy.ts`'s `operation.status.v1` handler stays live and integration-tested as a cross-version wire surface, because a successor coordinator build must be able to observe a proxy this build already started, and a responder cannot be retrofitted into a process that has already shipped. The coordinator layer is the only place allowed to compose multiple parent-daemon domains together and the only place that speaks to both transport and domain owner modules/contracts at once. `coordinator/handoff-runner.ts` owns the cross-build handoff decision and execution — probing a live incumbent's health, resolving the routing decision, and spawning/delegating into a validated newer build — shared by CLI pre-flight delegation, `wait jobs` cursor resume, and backend cold-start handoff (`src/coordinator/bootstrap.ts` calls it when startup finds a newer active-store selection).
