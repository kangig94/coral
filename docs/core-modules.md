...
The coordinator layer owns process lifecycle, the three-era boot sequence (Kernel → Recovery → Runtime Health Components), projection-consumer wiring, provider-host coordination, job-backed KB source import/reindex proxying, and the corpus notify seam. It also owns both halves of carrier observation that the classifier deliberately does not: `composition/carrier-observation.ts` gathers the local evidence (admission state, the OS process table, the durable-CLI identity row) and builds the wait stream's observation port, while `live/carrier-observer.ts` performs the bounded, identity-checked network probe of a foreign proxy. The split is enforced by reachability, not convention — `tests/invariants/no-carrier-observation-in-action-paths.test.ts` proves no recovery, cleanup, reconciliation, or terminal-materialization path can obtain a verdict at any import depth, and confines the network observer to the read-model and the composition that assembles it. It is the only place allowed to compose multiple parent-daemon domains together and the only place that speaks to both transport and domain owner modules/contracts at once. `coordinator/handoff-runner.ts` owns the cross-build handoff decision and execution — probing a live incumbent's health, resolving the routing decision, and spawning/delegating into a validated newer build — shared by CLI pre-flight delegation, `wait jobs` cursor resume, and backend cold-start handoff (`src/coordinator/bootstrap.ts` calls it when startup finds a newer active-store selection).
