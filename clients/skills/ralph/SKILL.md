...
<Ralph_Protocol>
  ...
  <Exec_Delegate>
    ...
    **Execution loop** — process batches from Execution Order sequentially; parallelize within each batch:
    ...
    2. `coral-cli wait jobs <job-id...> --embed` → each terminal block always prints `Result path: <path>`; read that path for the full artifact and use inline preview text only as a convenience. Exit `0` means every job succeeded (`completed`, or `provider_exit` whose child exited `0`); `1` means a failed, aborted, or faulted job ended the wait — if siblings from this batch are still running, the terminal block names them (`Run coral-cli wait jobs <ids> to continue waiting.`), and you must wait for those too (`coral-cli wait jobs <ids> --embed`) before proceeding, since they are still writing to the shared working tree; `75` means work is still running — resume with `coral-cli wait jobs <job-id...> --cursor <cursor> --embed` using the printed cursor, and keep looping until every job returns a non-`75` result. Do not proceed to step 3 while any job is still `75` or still named as a remaining job after a failure. A non-zero `provider_exit` code is the provider's own, passed through unchanged (0–255).
    ...
    2. Spawn N persistent workers (N = max parallel count from any batch in Execution Order).
       ...
       **If `--delegate`**: each worker's prompt must ALSO include these delegated-execution instructions
       ...
       1. `coral-cli <other-host> -b -i "<above structure + file paths + constraints>" --work-dir "<project root>" -d`
          → `coral-cli wait jobs <job> --embed` → the terminal output always includes `Result path: <path>`; read that path for the full artifact and treat inline preview text as optional convenience. Exit `0` means the run succeeded (`completed`, or `provider_exit` whose child exited `0`); `1` means a failed, aborted, or faulted job; `75` means work is still running — resume with `coral-cli wait jobs <job> --cursor <cursor> --embed` using the printed cursor, and keep looping until the job returns a non-`75` result. Do not proceed to step 2 while the job is still `75`. A non-zero `provider_exit` code is the provider's own, passed through unchanged (0–255).
